...
if ($is_admin) {
	...
	$query = "SELECT comment.CommentIndex, comment.Comment, comment.Strength " .
			 "       FROM comment " . "ORDER BY comment.CommentIndex";
	$res = & $db->query($query);
	if (DB::isError($res))
	...
	$newlink = "index.php?location=" . dbfuncString2Int("admin/comment/new.php") .
			 "&amp;next=" . dbfuncString2Int(
											"index.php?location=" .
											 dbfuncString2Int(
															"admin/comment/list.php"));
	$newbutton = dbfuncGetButton($newlink, "New comment", "medium", "", 
								"Create new comment");
	...
	if ($res->numRows() > 0) {
		...
		while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
			...
			$dellink = "index.php?location=" .
					 dbfuncString2Int("admin/comment/delete_confirm.php") .
			...
			$editlink = "index.php?location=" .
						 dbfuncString2Int("admin/comment/modify.php") .
						 "&amp;key=" . dbfuncString2Int($row['CommentIndex']) .
						 "&amp;next=" . dbfuncString2Int(
