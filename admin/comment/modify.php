<?php
	/*****************************************************************
	 * admin/comment/modify.php  (c) 2008 Jonathan Dieter
	 *
	 * Modify comment
	 *****************************************************************/

	/* Get variables */
	$title            = "Change comment #" . dbfuncInt2String($_GET['key']);
	$commentindex     = dbfuncInt2String($_GET['key']);
	$link             = "index.php?location=" . dbfuncString2Int("admin/comment/new_or_modify_action.php") .
						"&amp;key=" .           $_GET['key'] .
						"&amp;next=" .          $_GET['next'];
	...
	if($is_admin) {
		$fRes =& $db->query("SELECT Comment, Strength FROM comment " .
		...
		if($fRow =& $fRes->fetchRow(DB_FETCHMODE_ASSOC)) {
			...
			if(!isset($_POST['comment'])) {
				$_POST['comment'] = htmlspecialchars($fRow['Comment']);
			} else {
				$_POST['comment'] = htmlspecialchars($_POST['comment']);
			}
			if(!isset($_POST['strength'])) {
				if(is_null($fRow['Strength'])) {
					$_POST['strength'] = "NULL";
				} else {
					$_POST['strength'] = $fRow['Strength'];
				}
			}
			if(!isset($_POST['number'])) {
				$_POST['number'] = $commentindex;
			}

			if(!isset($values)) {
				$values = array();
				$query =	"SELECT SubjectTypeIndex FROM commenttype " .
							"WHERE CommentIndex = $commentindex";
				$res =&  $db->query($query);
			...
			echo "      <p align=\"center\">Can't find comment.  Have you deleted it?</p>\n";
			echo "      <p align=\"center\"><a href=\"$backLink\">Click here to go back</a></p>\n";
		}
		log_event($LOG_LEVEL_EVERYTHING, "admin/comment/modify.php", $LOG_ADMIN,
				"Opened comment #$commentindex for editing.");
