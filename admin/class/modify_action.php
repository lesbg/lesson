...
if ($is_admin) {
    ...
    if ($_POST["action"] == ">>") { // If >> was pressed, remove students from
        foreach ( $_POST['removefromclass'] as $remUserName ) { // class
            $res = & $db->query(
                    "DELETE FROM classlist " . "WHERE Username = '$remUserName' " .
        ...
        foreach ( $_POST['addtoclass'] as $addUserName ) { // class
            $res = & $db->query(
                    "INSERT INTO classlist (Username, ClassTermIndex) VALUES " .
                             "                      ('$addUserName', $classterm)");
            if (DB::isError($res))
                die($res->getDebugInfo()); // Check for errors in query
                                               // update_conduct_mark($addUserName // Where the flip do I find the term?
            log_event($LOG_LEVEL_ADMIN, "admin/class/modify_action.php",
                    $LOG_ADMIN, "Added $addUserName to $classname.");
        }
        update_classterm($classindex, $currentterm);
        ...
        $res = &  $db->query(
                    "SELECT ClassTeacherUsername FROM class " .
                             "WHERE ClassIndex=$classindex");
        if (DB::isError($res))
            die($res->getDebugInfo()); // Check for errors in query
        if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            if ($remUserName != "") {
                $res = &  $db->query(
                                "UPDATE class SET ClassTeacherUsername=NULL " .
                                     "WHERE ClassIndex=$classindex");
                if (DB::isError($res))
                    die($res->getDebugInfo()); // Check for errors in query
                log_event($LOG_LEVEL_ADMIN, "admin/class/modify_action.php", $LOG_ADMIN,
        ...
        if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
            ...
            $res = &  $db->query(
                            "UPDATE class SET ClassTeacherUsername='$addUserName' " .
                             "WHERE ClassIndex=$classindex");
            if (DB::isError($res))
                die($res->getDebugInfo()); // Check for errors in query
            if ($remUserName != "") {
    ...
    log_event($LOG_LEVEL_ERROR, "admin/class/modify_action.php",
