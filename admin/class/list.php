...
if ($is_admin or $is_principal or $is_counselor or $is_hod) {
    ...
    if ($res->numRows() > 0) {
        $pdflink = "index.php?location=" . dbfuncString2Int("admin/class/pdf.php") .
                            "&key="      . dbfuncString2Int("-1");
        $pdfbutton = dbfuncGetButton($pdflink, "Printable list", "medium", "", "Generate PDF of all classes in this department");
        echo " <p align='center'>$pdfbutton</p>\n";
        /*
         * if($is_admin) {
        ...
        while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
            ...
            if ($is_admin or $is_hod or $is_principal) {
                ...
                if ($row['CanDoReport'] or $row['ReportDone']) {
                    $rptlink = "index.php?location=" .
                         dbfuncString2Int("teacher/report/class_list.php") .
                         "&amp;key=" . dbfuncString2Int($row['ClassIndex']) .
                         "&amp;keyname=" . dbfuncString2Int($row['ClassName']);
                    if ($row['ReportDone']) {
                        $rptbutton = dbfuncGetButton($rptlink, "V", "small",
                                                    "report",
                                                    "View reports for class");
                    } else {
                        $rptbutton = dbfuncGetButton($rptlink, "R", "small",
                                                    "report",
                                                    "Edit reports for class");
                    }
                }
                $statlink = "index.php?location=" .
                             dbfuncString2Int("admin/statistics/class.php") .
                             "&amp;key=" .
                             dbfuncString2Int($row['ClassTermIndex']) .
                             "&amp;keyname=" .
                             dbfuncString2Int($row['ClassName']);
                $statbutton = dbfuncGetButton($statlink, "S", "small", "report",
                                            "View statistics for class");
            }
            if ($is_admin) {
                $editlink = "index.php?location=" .
                             dbfuncString2Int("admin/class/modify.php") .
                             "&amp;key=" . dbfuncString2Int($row['ClassIndex']) .
                             "&amp;keyname=" .
                             dbfuncString2Int($row['ClassName']);
            ...
            $pdflink = "index.php?location=" . dbfuncString2Int("admin/class/pdf.php") .
                                "&key="      . dbfuncString2Int($row['ClassIndex']);
            $pdfbutton = dbfuncGetButton($pdflink, "P", "small", "cn", "Generate PDF");

            echo "            <td>$viewbutton$editbutton$rptbutton$ttbutton$statbutton$pdfbutton</td>\n";
            ...
            if ($is_admin) {
                $dellink = "index.php?location=" .
                         dbfuncString2Int("admin/class/delete_confirm.php") .
                         "&amp;key=" . dbfuncString2Int($row['ClassIndex']) .
                         "&amp;keyname=" . dbfuncString2Int($row['ClassName']);
