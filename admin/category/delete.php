		...
		if($is_admin) {
			...
			if($iserror) {
				...
				while($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
					...
					/* Delete stuff */
					$dRes =& $db->query("DELETE FROM categorylist " .
										"WHERE CategoryIndex = $categoryindex " .
										"AND   SubjectIndex = $subjectindex");
					if(DB::isError($dRes)) die($dRes->getDebugInfo());
				...
				$res =&  $db->query("DELETE FROM category " .          // Remove category type from category table
									"WHERE CategoryIndex = $categoryindex");
				if(DB::isError($res)) die($res->getDebugInfo());          // Check for errors in query
			...
			log_event($LOG_LEVEL_ERROR, "admin/category/delete.php", $LOG_DENIED_ACCESS,
