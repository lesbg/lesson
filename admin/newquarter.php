...
$ttyres = & $db->query(
					"SELECT TermName, TermIndex FROM term " . // Run query to get term
					 "WHERE DepartmentIndex=$depindex " . "ORDER BY TermNumber");
if (DB::isError($ttyres))
	die($ttyres->getMessage()); // Check for errors in query
...
$query = "SELECT SubjectIndex, SubjectTypeIndex, Name FROM subject " .
		 "WHERE  YearIndex = $currentyear " . "AND    TermIndex = $currentterm " .
		 "ORDER BY Name";
$res = &  $db->query($query);
if (DB::isError($res))
...
while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
	...
	$chkRes = &   $db->query(
						"SELECT SubjectIndex FROM subject " .
						 "WHERE Name             = '{$row['Name']}' " .
						 "AND   SubjectTypeIndex = {$row['SubjectTypeIndex']} " .
						 "AND   YearIndex        = $currentyear " .
						 "AND   TermIndex        = $newterm");
	if (DB::isError($chkRes))
		die($chkRes->getDebugInfo()); // Check for errors in query
	...
	if (! ($chkRow = & $chkRes->fetchRow(DB_FETCHMODE_ASSOC))) {
		...
		if (DB::isError($newRes))
			die($newRes->getDebugInfo()); // Check for errors in query
		echo "..";
		...
		if (! $newRow = & $newRes->fetchRow(DB_FETCHMODE_ASSOC) or
			 $newRow['SubjectIndex'] == 0) {
			echo "<b>failed!</b></p>\n";
			continue;
		}
		...
		$frmRes = &   $db->query(
							"SELECT SubjectIndex, Username FROM subjectstudent " .
							 "WHERE SubjectIndex = {$row['SubjectIndex']}");
		if (DB::isError($frmRes))
			die($frmRes->getDebugInfo()); // Check for errors in query
		while ( $frmRow = & $frmRes->fetchRow(DB_FETCHMODE_ASSOC) ) {
			...
			if (DB::isError($stdRes))
				die($stdRes->getDebugInfo()); // Check for errors in query
			echo ".";
		}
		...
		if (DB::isError($frmRes))
			die($frmRes->getDebugInfo()); // Check for errors in query
		while ( $frmRow = & $frmRes->fetchRow(DB_FETCHMODE_ASSOC) ) {
			if (! isset($frmRow['ShowTeacher']))
				$frmRow['ShowTeacher'] = "NULL";
		...
		if (DB::isError($frmRes))
			die($frmRes->getDebugInfo()); // Check for errors in query
		while ( $frmRow = & $frmRes->fetchRow(DB_FETCHMODE_ASSOC) ) {
			if (! isset($frmRow['ClassIndex']))
				$frmRow['ClassIndex'] = "NULL";
			if (! isset($frmRow['TTNextIndex']))
				$frmRow['TTNextIndex'] = "NULL";
...
$res = &  $db->query(
				"SELECT DisciplineWeight, DisciplineTypeIndex " .
...
$chkRes = &   $db->query(
					"SELECT classterm.ClassTermIndex, newclassterm.ClassTermIndex AS " .
...
/* Change current term */
$nwCurRes = & $db->query(
						"UPDATE currentterm SET TermIndex = $newterm " .
						 "WHERE DepartmentIndex = $depindex");
if (DB::isError($nwCurRes))
	die($nwCurRes->getDebugInfo()); // Check for errors in query
