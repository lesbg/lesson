<?php
	...
	if(!isset($_POST["actiont"])) $_POST["actiont"] = "";
	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN)) {
		...
		if($_POST["action"] == ">" and isset($_POST['removefromsubject'])) {
			foreach($_POST['removefromsubject'] as $remUserName) {
				...
				$res =&  $db->query("SELECT user.FirstName, user.Surname, mark.Username FROM mark, assignment, user " .
				...
				if($res->numRows() > 0 && !$forceRemove) {       // If there's at least one mark with a score or comment,
					...
					$nRes =& $db->query("SELECT AssignmentIndex FROM assignment WHERE SubjectIndex = $subjectindex");
					...
					while($nRow =& $nRes->fetchRow(DB_FETCHMODE_ASSOC)) {      // This is a work-around for early (<4.0) versions
						$res =&  $db->query("DELETE FROM mark " .              //  of MySQL
											"WHERE mark.Username = '$remUserName' " .
											"AND   mark.AssignmentIndex = {$nRow['AssignmentIndex']}");
						if(DB::isError($res)) die($res->getDebugInfo());       // Check for errors in query
					}
					$res =&  $db->query("DELETE FROM subjectstudent " .
										"WHERE Username     = \"$remUserName\" " .
										"AND   SubjectIndex = $subjectindex");
					if(DB::isError($res)) die($res->getDebugInfo());           // Check for errors in query
					log_event($LOG_LEVEL_ADMIN, "admin/subject/modify_list_action.php", $LOG_ADMIN,
							"Removed $remUserName from subject $subject.");
				}
			}
			include "admin/subject/modify_list.php";
		} elseif($_POST["action"] == ">>") {                                    // If < was pressed, add students to
			$ares =& $db->query("SELECT user.FirstName, user.Surname, user.Username FROM " .
								"       user, subjectstudent " .
			...
			foreach($_POST['addtosubject'] as $addUserName) {                  //  subject
				$res =&  $db->query("SELECT Username FROM subjectstudent " .
									"WHERE Username     = \"$addUserName\" " .
									"AND   SubjectIndex = $subjectindex");
				if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
				if($res->numRows() == 0) {
					$res =& $db->query("INSERT INTO subjectstudent (Username, SubjectIndex) VALUES " .
									   "                           (\"$addUserName\", $subjectindex)"); 
					if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
					log_event($LOG_LEVEL_ADMIN, "admin/subject/modify_list_action.php", $LOG_ADMIN,
							"Added $addUserName to subject $subject.");
				}
			}
			include "admin/subject/modify_list.php";
			...
			foreach($_POST['removefromteacherlist'] as $remUserName) {       //  subject
				$res =&  $db->query("DELETE FROM subjectteacher " . 
									"WHERE Username     = \"$remUserName\" " .
									"AND   SubjectIndex = $subjectindex");
				if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
				log_event($LOG_LEVEL_ADMIN, "admin/subject/modify_list_action.php", $LOG_ADMIN,
						"Removed $remUserName from teaching subject $subject.");
			}
			include "admin/subject/modify_list.php";
			...
			foreach($_POST['addtoteacherlist'] as $addUserName) {         //  class
				...
				if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
				if($res->numRows() == 0) {
					$res =& $db->query("INSERT INTO subjectteacher (Username, SubjectIndex) VALUES " .
									   "                           (\"$addUserName\", $subjectindex)"); 
					if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
				}
				log_event($LOG_LEVEL_ADMIN, "admin/subject/modify_list_action.php", $LOG_ADMIN,
		...
		log_event($LOG_LEVEL_ERROR, "admin/subject/modify_list_action.php", $LOG_DENIED_ACCESS,
				"Attempted to modify subject $subject.");
