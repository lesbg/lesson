	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN)) {
		...
		if($_POST["action"] == ">>") {                                   // If >> was pressed, remove student from
			foreach($_POST['removesubject'] as $remSubject) {            //  subjects
				...
				$res =&  $db->query("SELECT Name FROM subject " .
				...
				$res =&  $db->query("SELECT user.FirstName, user.Surname, mark.Username, subject.Name " .
				...
				if($res->numRows() > 0 && !$forceRemove) {       // If there's at least one mark with a score or comment,
					...
					$nRes =& $db->query("SELECT AssignmentIndex FROM assignment WHERE SubjectIndex = $remSubject");
					...
					if(DB::isError($res)) die($res->getDebugInfo());           // Check for errors in query
					log_event($LOG_LEVEL_ADMIN, "admin/subject/modify_by_student_action.php", $LOG_ADMIN,
							"Removed $student from subject $subjectname.");
				}
			}
			...
			foreach($_POST['addsubject'] as $addSubject) {                  //  subject
				...
				if($res->numRows() == 0) {
					...
					if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
					log_event($LOG_LEVEL_ADMIN, "admin/subject/modify_by_student_action.php", $LOG_ADMIN,
							"Added $student to subject $subjectname.");
				}
			}
			update_conduct_mark($studentusername);
			include "admin/subject/modify_by_student.php";
		...
		log_event($LOG_LEVEL_ERROR, "admin/subject/modify_by_student_action.php", $LOG_DENIED_ACCESS,
