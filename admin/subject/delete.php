<?php
	...
	if($_POST['action'] == "Yes, delete subject") {
		...
		if(dbfuncGetPermission($permissions, $PERM_ADMIN)) {
			...
			$res =&  $db->query("SELECT Username FROM subjectstudent " .       // Check whether user to be deleted is in any class
			...
			if($iserror) {                                                // Check whether there have been any errors during the
				...
				$res =&  $db->query("DELETE FROM subjectteacher " .       // Remove any teachers from subject
									"WHERE SubjectIndex = $subjectindex");
				if(DB::isError($res)) die($res->getDebugInfo());          // Check for errors in query
				
				$nRes =& $db->query("SELECT AssignmentIndex, Title FROM assignment WHERE SubjectIndex = $subjectindex");
				...
				$res =&  $db->query("DELETE FROM assignment " .           // Remove all assignments for subject
									"WHERE SubjectIndex = $subjectindex");
				if(DB::isError($res)) die($res->getDebugInfo());          // Check for errors in query
				
				$res =&  $db->query("DELETE FROM subject " .              // Remove subject from subject table
									"WHERE SubjectIndex = $subjectindex");
				if(DB::isError($res)) die($res->getDebugInfo());          // Check for errors in query
			...
			log_event($LOG_LEVEL_ERROR, "admin/subject/delete.php", $LOG_DENIED_ACCESS,
