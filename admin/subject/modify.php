<?php
	/*****************************************************************
	 * admin/subject/modify.php  (c) 2005-2008 Jonathan Dieter
	 *
	 * Change information about subject
	 *****************************************************************/

	/* Get variables */
	$title            = "Change " . dbfuncInt2String($_GET['keyname']);
	$subjectindex     = dbfuncInt2String($_GET['key']);
	$link             = "index.php?location=" . dbfuncString2Int("admin/subject/new_or_modify_action.php") .
						"&amp;key=" .           $_GET['key'] .
	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN)) {
		...
		$query =	"SELECT SubjectTypeIndex, Name, ClassIndex, Grade, " .
					"       EffortType, AverageType, ConductType, " .
		...
		if($fRow =& $fRes->fetchRow(DB_FETCHMODE_ASSOC)) {
			if(isset($errorlist)) {                           // If there were errors, print them, and reset fields
				...
				$_POST['name']         = htmlspecialchars($fRow['Name']);
				$_POST['conduct_type'] = intval($fRow['ConductType']);
				$_POST['effort_type']  = intval($fRow['EffortType']);
				$_POST['average_type'] = intval($fRow['AverageType']);
				$_POST['subjecttype']  = intval($fRow['SubjectTypeIndex']);
				$_POST['categories']   = intval($fRow['TeacherCanChangeCategories']);
				if(!is_null($fRow['ClassIndex'])) {
					$_POST['class'] = intval($fRow['ClassIndex']);
				} else {
					$_POST['class'] = NULL;
				}
				if(!is_null($fRow['Grade'])) {
			...
			$res =&  $db->query("SELECT SubjectTypeIndex, Title FROM subjecttype ORDER BY Title");
			...
			while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
				...
				if($row['SubjectTypeIndex'] == $_POST['subjecttype']) echo " selected";
				echo ">{$row['Title']}\n";
			}
			echo "               </select></td>\n";
			echo "            </tr>\n";
			...
			echo "            <tr>\n";
			echo "               <td>Period</td>\n";
			echo "               <td><input type=\"text\" name=\"period\" value=\"{$_POST['period']}\" size=35></td>\n";
			echo "            </tr>\n";
			
			/* Show list of classes */
			$res =&  $db->query("SELECT ClassName, ClassIndex FROM class WHERE YearIndex=$yearindex AND DepartmentIndex=$depindex ORDER BY Grade");
			if(DB::isError($res)) die($res->getDebugInfo());           // Check for errors in query
			echo "            <tr>\n";
			echo "               <td>Class</td>\n";
			echo "               <td><select name=\"class\">\n";
			echo "                  <option value=\"NULL\"";
			if(is_null($_POST['class'])) echo " selected";
			echo ">(No specific class)\n";
			while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
				echo "                  <option value=\"{$row['ClassIndex']}\"";
				if($row['ClassIndex'] == $_POST['class']) echo " selected";
				echo ">{$row['ClassName']}\n";
			}
			echo "               </select></td>\n";
			echo "            </tr>\n";
			...
			/* Show list of grades */
			$res =&  $db->query("SELECT GradeName, Grade FROM grade WHERE DepartmentIndex=$depindex ORDER BY Grade");
			if(DB::isError($res)) die($res->getDebugInfo());           // Check for errors in query
			echo "            <tr>\n";
			echo "               <td>Grade</td>\n";
			echo "               <td><select name=\"grade\">\n";
			echo "                  <option value=\"NULL\"";
			if(is_null($_POST['grade'])) echo " selected";
			echo ">(No specific grade)\n";
			while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
				echo "                  <option value=\"{$row['Grade']}\"";
				if($row['Grade'] == $_POST['grade']) echo " selected";
				echo ">{$row['GradeName']}\n";
			}
			echo "               </select></td>\n";
			echo "            </tr>\n";
		...
		log_event($LOG_LEVEL_EVERYTHING, "admin/subject/modify.php", $LOG_ADMIN,
				"Opened $title for editing.");
	} else {  // User isn't authorized to view or change scores.
		/* Log unauthorized access attempt */
		log_event($LOG_LEVEL_ERROR, "admin/subject/modify.php", $LOG_DENIED_ACCESS,
