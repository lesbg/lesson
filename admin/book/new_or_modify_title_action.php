<?php
	...
	if(isset($_POST['action']) and ($_POST['action'] == "Save" or $_POST['action'] == "Update")) {   // If update or save were pressed, print
	...
	} elseif(isset($_POST[$check_subject_key]) and $_POST[$check_subject_key] == "Remove") {
	...
	// User added subject to book
		...
		if(isset($_POST['subject']) and !is_null($_POST['subject']) and $_POST['subject'] != "NULL") {
	...
	} elseif(isset($_POST[$check_class_key]) and $_POST[$check_class_key] == "Remove") {
		$check_key = intval(str_replace('del_class_', '', $check_class_key));
		$query =	"DELETE FROM book_class " .
					"WHERE  BookClassIndex = $check_key";
		$aRes =& $db->query($query);
		if(DB::isError($aRes)) die($aRes->getDebugInfo());           // Check for errors in query
		include "admin/book/modify_title.php";
	...
	// User added class to book
		...
		if(isset($_POST['class']) and !is_null($_POST['class']) and $_POST['class'] != "NULL" and 
		   isset($_POST['class_special']) and intval($_POST['class_special']) >= 0 and intval($_POST['class_special']) < 3) {
	...
	// User added grade to book
		...
		if(isset($_POST['grade']) and !is_null($_POST['grade']) and $_POST['grade'] != "NULL" and
		   isset($_POST['grade_special']) and intval($_POST['grade_special']) >= 0 and intval($_POST['grade_special']) < 3) {
