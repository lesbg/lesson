<?php
	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN)) {           // Make sure user has permissions
		log_event($LOG_LEVEL_EVERYTHING, "admin/viewlog.php", $LOG_ADMIN, "Viewed log.");
		...
		$sessionDec = dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=0", "D", "small", "{$sort[0]}", "Sort descending");
		$unameAsc =   dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=2", "A", "small", "{$sort[2]}", "Sort ascending");
		$unameDec =   dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=3", "D", "small", "{$sort[3]}", "Sort descending");
		$levelAsc =   dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=4", "A", "small", "{$sort[4]}", "Sort ascending");
		$levelDec =   dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=5", "D", "small", "{$sort[5]}", "Sort descending");
		$codeAsc =    dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=6", "A", "small", "{$sort[6]}", "Sort ascending");
		$codeDec =    dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=7", "D", "small", "{$sort[7]}", "Sort descending");
		$commentAsc = dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=8", "A", "small", "{$sort[8]}", "Sort ascending");
		$commentDec = dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=9", "D", "small", "{$sort[9]}", "Sort descending");
		$pageAsc =    dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=10", "A", "small", "{$sort[10]}", "Sort ascending");
		$pageDec =    dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=11", "D", "small", "{$sort[11]}", "Sort descending");
		$rhostAsc =   dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                              "&amp;sort=12", "A", "small", "{$sort[12]}", "Sort ascending");
		$rhostDec =   dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		...
		$res =&  $db->query("SELECT user.FirstName, user.Surname, log.Username, log.Time, log.Session, " .
		...
		if(intval($start) > 0) {
			$first_record = dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                                    "&amp;sort={$_GET['sort']}&amp;start=0", "<<", "medium", "prevnext", "First page");
			$prev         = intval($start) - $LOGS_PER_PAGE;
			if($prev < 0)
				$prev     = 0;
			$prev         = strval($prev);
			$prev_record  = dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		...
		if(intval($start) < $fpstart) {
			$last_record  = dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		                                    "&amp;sort={$_GET['sort']}&amp;start=$fpstart", ">>", "medium", "prevnext", "Last page");
			$next         = intval($start) + $LOGS_PER_PAGE;
			if($next > $fpstart)
				$next     = $fpstart;
			$next         = strval($next);
			$next_record  = dbfuncGetButton("index.php?location=" . dbfuncString2Int("admin/viewlog.php") .
		...
		log_event($LOG_LEVEL_ERROR, "admin/viewlog.php", $LOG_DENIED_ACCESS, "Tried to view log.");
