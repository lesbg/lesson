...
if ($_POST['action'] == "Ok") {
    ...
    if ($is_admin or $is_principal) {
        if (isset($_POST['teacher']) and $_POST['teacher'] != "" and
            ...
            $aRes = & $db->query(
                                "UPDATE department SET ProofreaderUsername='{$_POST['teacher']}' " .
                                 "WHERE  DepartmentIndex = $choice_depindex");
            if (DB::isError($aRes))
                die($aRes->getDebugInfo());
        } else {
            $aRes = & $db->query(
                                "UPDATE department SET ProofreaderUsername=NULL " .
                                 "WHERE  DepartmentIndex = $choice_depindex");
            if (DB::isError($aRes))
                die($aRes->getDebugInfo());
        }
        log_event($LOG_LEVEL_ADMIN, "admin/proofreader/modify_action.php",
                $LOG_ADMIN,
                "Changed proofreader for $choice_department department.");
    } else {
        /* Log unauthorized access attempt */
        log_event($LOG_LEVEL_ERROR, "admin/proofreader/modify_action.php",
                $LOG_DENIED_ACCESS,
