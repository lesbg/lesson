<?php
	...
	$link             = "index.php?location=" . dbfuncString2Int("admin/proofreader/modify_action.php") .
						"&amp;key=" .           $_GET['key'] .
	...
	if($is_admin or $is_principal) {
		...
		/* Get teacher list */
		$res =&  $db->query("SELECT user.Title, user.FirstName, user.Surname, user.Username " .
							"       FROM user " .
		...
		if($res->numRows() > 0) {
			...
			$default = "";
			if($proof_username == "") $default = "checked";
			...
			while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
				...
				$default = "";
				if($row['Username'] == $proof_username) $default = "checked";
