...
if ($is_admin) {	
	...
	if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
		...
		$query = "SELECT GroupMemberIndex, GroupID FROM groupmem WHERE Member='{$_POST['uname']}'";
		$aRes = & $db->query($query);
		if (DB::isError($aRes))
			die($aRes->getDebugInfo()); // Check for errors in query
		while ( $arow = & $aRes->fetchRow(DB_FETCHMODE_ASSOC) ) {
		...
		foreach($_POST['groups'] as $group) {
			$query = "SELECT GroupMemberIndex, GroupID FROM groupmem WHERE Member='{$_POST['uname']}' AND GroupID='$group'";
			$aRes = & $db->query($query);
			if (DB::isError($aRes))
				die($aRes->getDebugInfo()); // Check for errors in query
			if ($aRes->numRows() == 0) {
				$query = "INSERT INTO groupmem (Member, GroupID) VALUES ('{$_POST['uname']}', '$group')";
				$aRes = & $db->query($query);
				if (DB::isError($aRes))
					die($aRes->getDebugInfo()); // Check for errors in query
			}
