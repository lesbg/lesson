<?php
	...
	if($_POST['action'] == "Yes, delete user") {
		...
		if($is_admin) {
			...
			$res =&  $db->query("SELECT Username FROM classlist " .       // Check whether user to be deleted is in any class
			...
			$res =&  $db->query("SELECT Username FROM subjectstudent " .  // Check whether user to be deleted is in any subjects
								"WHERE Username  = '$delusername'");
			if(DB::isError($res)) die($res->getDebugInfo());              // Check for errors in query
			if($res->numRows() > 0) {
				$errorname .= "      <p align=\"center\">You cannot delete $delfullname until you remove them from all of " .
			...
			$res =&  $db->query("SELECT Username FROM subjectteacher " .  // Check whether user to be deleted teaches any subjects
								"WHERE Username  = '$delusername'");
			if(DB::isError($res)) die($res->getDebugInfo());              // Check for errors in query
			if($res->numRows() > 0) {
			...
			if($iserror) {                                                // Check whether there have been any errors during the
				...
				$res =&  $db->query("DELETE FROM user " .             // Remove user from user table
									"WHERE Username  = '$delusername'");
				if(DB::isError($res)) die($res->getDebugInfo());          // Check for errors in query
