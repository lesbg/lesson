<?php
	...
	$link            = "index.php?location=" . dbfuncString2Int("admin/user/new_or_modify_action.php") .
					   "&amp;key=" .           $_GET['key'] .
	...
	if($is_admin) {
		...
		if($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
			$sexM = ""; $sexF = "";                                    // Check sex of user and set appropriate
			if($row['Gender'] == 'M')                                  //  variables
				$sexM = "checked";
			else
				$sexF = "checked";
			
			$dateTypeD = ""; $dateType0 = ""; $dateType1 = "";         // Check date type of user and set appropriate
			...
			$dateSepD = ""; $dateSep0 = "";                            // Check date separator of user and set appropriate
			$dateSep1 = ""; $dateSep2 = "";                            //  variables
			if($row['DateSep'] == "/")
				$dateSep0 = "checked";
			elseif($row['DateSep'] == "-")
				$dateSep1 = "checked";
			...
			$check1 = "";        $check2 = "";
			if($row['ActiveStudent'] == 1) $activeStudent = "checked";
			if($row['ActiveTeacher'] == 1) $activeTeacher = "checked";
			if($row['SupportTeacher'] == 1) $supportTeacher = "checked";
			
			if($row['User1'] == 1)         $check1 = "checked";
			if($row['User2'] == 1)         $check2 = "checked";
