<?php
	...
	if($_POST["action"] == "Save" || $_POST["action"] == "Update") {   // If update or save were pressed, print
		...
		$format_error = False;
		$errorlist    = "";
		if(!isset($_POST['title']) || is_null($_POST['title']) || $_POST['title'] == "") {  // Make sure name has been entered
			$errorlist      .= "<p class=\"error\" align=\"center\">You must specifiy a subject type name!</p>\n";
			$format_error    = True;
			log_event($LOG_LEVEL_ADMIN, "admin/subjecttype/new_or_modify_action.php", $LOG_ERROR,
					"Tried to set subject type information, but forgot to specify a subject type name.");
		}
			
		if(!$format_error){
			$_POST['title'] = "'" . $db->escapeSimple($_POST['title']) . "'";
			if(!isset($_POST['descr']) || is_null($_POST['descr']) || $_POST['descr'] == "") {
				$_POST['descr'] = "NULL";
			} else {
