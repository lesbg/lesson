<?php
	...
	if($_POST['action'] == "Yes, delete subject type") {
		...
		if(dbfuncGetPermission($permissions, $PERM_ADMIN)) {
			...
			$res =&  $db->query("SELECT SubjectIndex FROM subject " .     // Check whether subjecttype to be deleted has any subjects
			...
			if($res->numRows() > 0) {
				...
				$iserror    = True;
				log_event($LOG_LEVEL_ADMIN, "admin/subjecttype/delete.php", $LOG_ERROR,
			...
			if($iserror) {                                                // Check whether there have been any errors during the
				...
				$res =&  $db->query("DELETE FROM subjecttype " .          // Remove subject type from subjecttype table
									"WHERE SubjectTypeIndex = $subjecttypeindex");
				if(DB::isError($res)) die($res->getDebugInfo());          // Check for errors in query
				
				echo "      <p align=\"center\">$subjecttype successfully deleted.</p>\n";
				log_event($LOG_LEVEL_ADMIN, "admin/subjecttype/delete.php", $LOG_ADMIN,
						"Deleted subject type $subjecttype.");
			}
			echo "      <p align=\"center\"><a href=\"$nextLink\">Continue</a></p>\n";
		} else {
			log_event($LOG_LEVEL_ERROR, "admin/subjecttype/delete.php", $LOG_DENIED_ACCESS,
