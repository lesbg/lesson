...
if (dbfuncGetPermission($permissions, $PERM_ADMIN)) { // Make sure user has permission to view and
	...
	$res = &  $db->query(
					"SELECT Title, Description, SubjectTypeIndex FROM subjecttype " .
					 "ORDER BY Title");
	if (DB::isError($res))
	...
	$newlink = "index.php?location=" .
			 dbfuncString2Int("admin/subjecttype/new.php"); // link to create a new subject
	$newbutton = dbfuncGetButton($newlink, "New subject type", "medium", "", 
	...
	if ($res->numRows() > 0) {
		...
		while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
			...
			$editlink = "index.php?location=" .
						 dbfuncString2Int("admin/subjecttype/modify.php") .
						 "&amp;key=" .
						 dbfuncString2Int($row['SubjectTypeIndex']) .
