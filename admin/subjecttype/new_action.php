...
if (dbfuncGetPermission($permissions, $PERM_ADMIN)) {
	...
	$res = & $db->query(
					"SELECT SubjectTypeIndex FROM subjecttype WHERE Title = {$_POST['title']}");
	if (DB::isError($res))
	...
	if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
		...
		$aRes = & $db->query(
							"INSERT INTO subjecttype (Title, Description) " .
	...
	log_event($LOG_LEVEL_ERROR, "admin/subjecttype/new_action.php", 
			$LOG_DENIED_ACCESS, 
