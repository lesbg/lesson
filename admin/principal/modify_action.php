	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN)) {
		...
		if($_POST["action"] == ">") {
			foreach($_POST['remove'] as $remUserName) {
				$res =&  $db->query("DELETE FROM principal " .
			...
			foreach($_POST['add'] as $addUserName) {
				...
				if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
				if($res->numRows() == 0) {
					$res =& $db->query("INSERT INTO principal (Username, Level) VALUES " .
									   "                           (\"$addUserName\", $level)"); 
					if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
				}
				log_event($LOG_LEVEL_ADMIN, "admin/principal/modify_action.php", $LOG_ADMIN,
						"Set $addUserName as a $plevel.");
			}
			include "admin/principal/modify.php";
		...
		log_event($LOG_LEVEL_ERROR, "admin/principal/modify_action.php", $LOG_DENIED_ACCESS,
				"Attempted to modify principals.");
