<?php
	...
	if($_POST["action"] == "Send") {
		...
		include "header.php";                                        // Print header
				/* Get teacher list */
		$res =&  $db->query("SELECT user.PhoneNumber FROM user " .
							"WHERE Username=\"$destusername\"");
		if(DB::isError($res)) die($res->getDebugInfo());           // Check for errors in query
			
		if ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC) and !is_null($row['PhoneNumber']) and $row['PhoneNumber'] != "") {
			if($_POST['text'] != "") {
				$text = escapeshellarg($_POST['text']);
				$retval = 0;
				$retarray = array();
				echo "      <p align=\"center\">Sending SMS to $destfullname...";
				exec("/usr/bin/sms {$row['PhoneNumber']} $SMS_PASSWORD \"$text\"", $retarray, $retval);
				if($retval == 0) {
					echo "done.<p>\n";
				} else {
					echo "failed!</p>\n";
