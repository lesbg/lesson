<?php
	...
	if($is_admin or $perm >= $PUN_PERM_PROXY) {
		...
		if($_POST["action"] == ">") {                                    // If > was pressed, remove selected students from
			...
			$res =&  $db->query("SELECT ClassName FROM class " .
			...
			if($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
				...
				if($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
					...
					if($failed == 0) {
							$punish_list[] = array(	"display"=>"Class $classname - " .
															"$dtype - $ttitle $tsurname - {$_POST['date']}",
													"class"=>$classindex,
													"teacher"=>$tusername,
													"issuedate"=>$dateinfo,
					...
					if($punishment != "") {
						...
						if($punishment['type'] == 1) {
							...
							while ($row =& $pres->fetchRow(DB_FETCHMODE_ASSOC)) {
								...
								log_event($LOG_LEVEL_ADMIN, "admin/punishment/proxy_action.php", $LOG_ADMIN,
									"Issued $dtype to {$row['Username']} on behalf of $tusername.");
							}
						} else {
							$studentusername = $db->escapeSimple($punishment['student']);
							$query =	"INSERT INTO discipline (DisciplineWeightIndex, Username, WorkerUsername, " .
										"                        RecordUsername, DateRequested, DateIssued, " .
										"                        Date, Comment) " .
										"       VALUES " .
		...
		log_event($LOG_LEVEL_ERROR, "admin/punishment/proxy_action.php", $LOG_DENIED_ACCESS,
				"Attempted to issue proxy punishment.");
