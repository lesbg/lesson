...
/**
 * ***************************************************************
 * admin/punishment/mass_action.php (c) 2006-2016 Jonathan Dieter
 *
...
if (dbfuncGetPermission($permissions, $PERM_ADMIN) or
	 ($perm >= $PUN_PERM_MASS and $is_teacher)) {
	...
	if ($_POST["action"] == ">") { // If > was pressed, remove selected students from
		...
		foreach ( $punish_list as $studentusername => $student ) {
			if (substr($studentusername, 0, 1) == "!") {
				$classindex = intval(
									substr(strrchr($studentusername, "!"), 1));
...
"        )";
		...
		while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
			$cuname = $row['Username'];
			if (isset($punish_list[$cuname]))
				unset($punish_list[$cuname]);
		}
		$res = &  $db->query(
						"SELECT ClassIndex, ClassName, Grade FROM class " .
							...
							if (substr($studentusername, 0, 1) == "!") {
								...
								while ( $row = & $pres->fetchRow(DB_FETCHMODE_ASSOC) ) {
									...
									log_event($LOG_LEVEL_ADMIN, 
											"admin/punishment/mass_action.php", 
											$LOG_ADMIN, 
	...
	log_event($LOG_LEVEL_ERROR, "admin/punishment/mass_action.php", 
			$LOG_DENIED_ACCESS, "Attempted to issue mass punishment.");
