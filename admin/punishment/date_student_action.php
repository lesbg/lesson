...
/**
 * ***************************************************************
 * admin/punishment/date_student_action.php (c) 2006-2016 Jonathan Dieter
 *
...
if (dbfuncGetPermission($permissions, $PERM_ADMIN) or
    ...
    if ($_POST["action"] == "Done" or $_POST["action"] == "Edit") {
        ...
        if (DB::isError($res))
            die($res->getDebugInfo()); // Check for errors in query
        foreach ( $_POST['mass'] as $dindex ) {
            $dindex = intval($dindex);

            $query = "UPDATE discipline SET DisciplineDateIndex=$pindex, ServedType=NULL " .
        ...
        if ($_POST["action"] == "Done") {
            log_event($LOG_LEVEL_ADMIN,
                    "admin/punishment/date_student_action.php", $LOG_ADMIN,
    ...
    log_event($LOG_LEVEL_ERROR, "admin/punishment/date_student_action.php",
            $LOG_DENIED_ACCESS, "Attempted to set punishment date.");
