...
/**
 * ***************************************************************
 * admin/punishment/proxy.php (c) 2006-2016 Jonathan Dieter
 *
...
$link = "index.php?location=" .
         dbfuncString2Int("admin/punishment/proxy_action.php") . "&amp;next=" .
         $_GET['next'];
...
if ($is_admin or ($perm >= $PUN_PERM_PROXY and $is_teacher)) {
    ...
    $query =    "SELECT user.Title, user.FirstName, user.Surname, user.Username FROM " .
                "       user INNER JOIN groupgenmem ON (user.Username=groupgenmem.Username) " .
                "            INNER JOIN groups USING (GroupID) " .
    ...
    log_event($LOG_LEVEL_ERROR, "admin/punishment/proxy.php",
            $LOG_DENIED_ACCESS,
