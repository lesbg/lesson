<?php
	/*****************************************************************
	 * admin/punishment/pending_confirm.php  (c) 2006-2013 Jonathan Dieter
	 *
	...
	$link =	"index.php?location=" . dbfuncString2Int("admin/punishment/pending.php") .
			"&amp;next=" .          $_GET['next'];
	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN) or ($perm >= $PUN_PERM_APPROVE and $is_teacher)) {
		...
		log_event($LOG_LEVEL_ERROR, "admin/punishment/pending_confirm.php", $LOG_DENIED_ACCESS,
				"Tried to approve or reject punishments.");
