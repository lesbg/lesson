<?php
	/*****************************************************************
	 * admin/punishment/set_date_action.php  (c) 2006 Jonathan Dieter
	 *
	 * Set punishment date
	 *****************************************************************/
	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN) or $perm >= $PUN_PERM_ALL) {
		...
		if($_POST["action"] == "Set punishment date")  {
			...
			$link = "index.php?location=" .	dbfuncString2Int("admin/punishment/date_student.php") .
							"&amp;next=" .	$_GET['next'] .
							"&amp;type=" .	$_GET['type'];
			...
			if(!isset($_POST['pundate']) or is_null($_POST['pundate']) or $_POST['pundate'] == "") {
				$pundate =& dbfuncCreateDate(date($dateformat));
			} else {
				$pundate =& dbfuncCreateDate($_POST['pundate']);
			}
			if(!isset($_POST['enddate']) or is_null($_POST['enddate']) or $_POST['enddate'] == "") {
		...
		log_event($LOG_LEVEL_ERROR, "admin/punishment/set_date_action.php", $LOG_DENIED_ACCESS,
				"Attempted to set up punishment date.");
