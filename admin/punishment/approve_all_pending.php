	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN) or $perm >= $PUN_PERM_APPROVE) {
		if($_POST['action'] == "Yes, approve all punishments") {
			...
			while($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
				...
				$nrs =&  $db->query("SELECT DisciplineWeightIndex FROM disciplineweight " .
		...
		log_event($LOG_LEVEL_ERROR, "admin/punishment/approve_all_pending.php", $LOG_DENIED_ACCESS,
				"Tried to approve $log_pun for $name.");
