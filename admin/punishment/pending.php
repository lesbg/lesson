...
/**
 * ***************************************************************
 * admin/punishment/pending.php (c) 2006-2016 Jonathan Dieter
 *
...
if (dbfuncGetPermission($permissions, $PERM_ADMIN) or
        ...
        foreach ( $_POST['mass'] as $backlogindex ) {
            ...
            if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
                ...
                if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
                    ...
                    log_event($LOG_LEVEL_ADMIN, "admin/punishment/pending.php",
                            $LOG_ADMIN, "$Did $dowhat$log_pun for $name.");
                }
            }
        }
    ...
    log_event($LOG_LEVEL_ERROR, "admin/punishment/pending.php",
            $LOG_DENIED_ACCESS, "Tried to $do punishments.");
