<?php
	...
	if(($username = $ttusername or $is_admin) and $type != "c") {
		...
		$nrs =&  $db->query("SELECT Username FROM user WHERE Username='$ttusername' AND " .
					"ActiveTeacher=1");
		if(DB::isError($res)) die($res->getDebugInfo());          // Check for errors in query
		if($nrs->numRows() > 0) {
			if($type == NULL) $type = dbfuncString2Int("t"); else $type = dbfuncString2Int($type);
		...
		if(DB::isError($res)) die($res->getDebugInfo());          // Check for errors in query
		if($nrs->numRows() > 0) {
			if($type == NULL) $type = dbfuncString2Int("s"); else $type = dbfuncString2Int($type);
			echo "   <center><embed src='svg/timetable.svg.php?key={$_GET['key']}&amp;key2=$type' type='image/svg+xml' width='800' height='400' pluginspage='http://www.adobe.com/svg/viewer/install/' /></center>\n";
		}
