<?php
	...
	if($_POST["action"] == "Ok") {                                   // If ok was pressed, try to change password
		...
		if(isset($_POST["new"]) and strtoupper($_POST["new"]) == strtoupper($username)) {
			$error = true;
		...
		if($res->NumRows() > 0) {
			if(strlen($_POST["new"]) >= 6) {
				if($_POST["new"] == $_POST["confirmnew"]) {
					$res =& $db->query("UPDATE user SET $pass_str = MD5(\"{$_POST['new']}\")" . 
									"WHERE Username = \"$username\"");
					if(DB::isError($res)) die($res->getDebugInfo());         // Check for errors in query
					echo "done.</p>\n";
					log_event($LOG_LEVEL_ADMIN, "user/dochangepassword.php", $LOG_USER,
							"Changed password $password_number.");
				} else {
					echo "failed!</p>\n";
					echo "      <p align=\"center\">The new password didn't match the confirm new password!</p>\n";
					log_event($LOG_LEVEL_EVERYTHING, "user/dochangepassword.php", $LOG_ERROR,
				...
				log_event($LOG_LEVEL_EVERYTHING, "user/dochangepassword.php", $LOG_ERROR,
						"The new password wasn't long enough.");
			}
		} else {
			echo "failed!</p>\n";
