...
/**
 * ***************************************************************
 * teacher/report/modify.php (c) 2008, 2018 Jonathan Dieter
 *
...
while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
    ...
    if ($is_st or $is_ct or $is_hod or $is_principal or $is_admin) {
        ...
        if ($average_type != $AVG_TYPE_NONE) {
            ...
            if (($row['ReportDone'] == 0 or $student_username != "") and
                 ($average_type == $AVG_TYPE_INDEX)) {
    ...
    if ($comment_type != $COMMENT_TYPE_NONE) {
        if ($comment_type == $COMMENT_TYPE_MANDATORY or
            ...
            if (isset($_POST["comment_{$row['Username']}"])) {
                $commentstr = htmlspecialchars(
                                            $_POST["comment_{$row['Username']}"],
                                            ENT_QUOTES);
            } else {
