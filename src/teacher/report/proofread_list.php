...
/**
 * ***************************************************************
 * teacher/report/proofread_list.php (c) 2008, 2017 Jonathan Dieter
 *
...
$query = "SELECT user.Gender, user.FirstName, user.Surname, user.Username, " .
         ...
         "       classlist.ReportProofDone, term.TermIndex, " .
         "       classlist.ReportPrinted, classterm.CTCommentType, " .
...
while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
    ...
    $link = "index.php?location=" .
             ...
             dbfuncString2Int($row['TermIndex']) . "&amp;keyname=" .
             dbfuncString2Int($row['ClassName']) . "&amp;keyname2=" .
             dbfuncString2Int($name);

    echo "               <td><a href='$link'>$name</a></td>\n";

    if ($row['CTCommentType'] == $COMMENT_TYPE_MANDATORY or
         $row['CTCommentType'] == $COMMENT_TYPE_OPTIONAL) {
    ...
    if ($row['HODCommentType'] == $COMMENT_TYPE_MANDATORY or
         $row['HODCommentType'] == $COMMENT_TYPE_OPTIONAL) {
    ...
    if ($row['PrincipalCommentType'] == $COMMENT_TYPE_MANDATORY or
         $row['PrincipalCommentType'] == $COMMENT_TYPE_OPTIONAL) {
