...
/**
 * ***************************************************************
 * teacher/punishment/request/list.php (c) 2006, 2018 Jonathan Dieter
 *
...
$title = "Pending Punishments issued by $name ($teacher_username)";
...
if (!$is_admin and $teacher_username != $username) {
    ...
    log_event($LOG_LEVEL_ERROR, "teacher/punishment/request/list.php",
            $LOG_DENIED_ACCESS,
...
/* Punishment requests */
$query->prepare(
    "SELECT disciplinetype.DisciplineType, user.Username, disciplinebacklog.DateOfViolation, " .
    ...
    "WHERE  disciplinebacklog.WorkerUsername   = :teacher_username " .
...
$query->execute(['teacher_username' => $teacher_username,
                 'termindex' => $termindex, 'yearindex' => $yearindex]);
$data = $query->fetchAll();
...
if($data) {
    ...
    foreach($data as $row) {
        ...
        $dellink = "index.php?location=" .
                 ...
                 "&amp;key=" .
                 dbfuncString2Int($row['DisciplineBacklogIndex']) .
                 "&amp;next=" .
                 dbfuncString2Int(
                                "index.php?location=" .
                                 dbfuncString2Int(
                                                "teacher/punishment/request/list.php") .
                                 "&key={$_GET['key']}&keyname={$_GET['keyname']}");
        $delbutton = dbfuncGetButton($dellink, "D", "small", "delete",
                                    "Delete pending punishment");
        ...
        $dellink = "index.php?location=" .
                 ...
                 dbfuncString2Int(
                                "index.php?location=" .
                                 ...
                                 "&key={$_GET['key']}&keyname={$_GET['keyname']}");
        $delbutton = dbfuncGetButton($dellink, "D", "small", "delete",
                                    "Delete pending punishment removal");
...
log_event($LOG_LEVEL_EVERYTHING, "teacher/punishment/request/list.php",
        $LOG_TEACHER,
