...
$query = "SELECT classterm.Average, classterm.Effort, classterm.Conduct, " .
         ...
         "       FROM classterm, class, classlist, report " .
...
$orig_data = substr($orig_data, strlen($header),
                    strlen($orig_data) -
...
while ( $student_info = & $sres->fetchRow(DB_FETCHMODE_ASSOC) ) {
    ...
    if ($average_type == $CLASS_AVG_TYPE_NONE) {
        $average = "-";
    } elseif ($average_type == $CLASS_AVG_TYPE_PERCENT or
    ...
    if ($effort_type == $CLASS_EFFORT_TYPE_NONE) {
        $effort = "-";
    } elseif ($effort_type == $CLASS_EFFORT_TYPE_PERCENT or
    ...
    if ($conduct_type == $CLASS_CONDUCT_TYPE_NONE) {
        $conduct = "-";
    } elseif ($conduct_type == $CLASS_CONDUCT_TYPE_PERCENT or
             $conduct_type == $CLASS_CONDUCT_TYPE_CALC or
             $conduct_type == $CLASS_CONDUCT_TYPE_PERCENT or
             $conduct_type == $CLASS_CONDUCT_TYPE_CALC or
    ...
    $groups = array();
    $query =    "SELECT groups.GroupID, groups.GroupName FROM groups, groupgenmem " .
    ...
    while ( $cRow = & $cRes->fetchrow(DB_FETCHMODE_ASSOC) ) {
        ...
        if ($cRow['AverageType'] == $CLASS_AVG_TYPE_PERCENT or
             $cRow['AverageType'] == $CLASS_AVG_TYPE_CALC) {
            ...
            if ($cRow['ClassAverage'] != - 1 and
                 ! is_null($cRow['ClassAverage'])) {
        ...
        if ($cRow['ConductType'] == $CLASS_CONDUCT_TYPE_PERCENT or
             $cRow['ConductType'] == $CLASS_CONDUCT_TYPE_CALC or
             $cRow['ConductType'] == $CLASS_CONDUCT_TYPE_PUN) {
            ...
            if ($cRow['ClassConduct'] != - 1 and
                 ! is_null($cRow['ClassConduct'])) {
        ...
        if ($pr_comment_type == $COMMENT_TYPE_NONE) {
            $pr_comment = "-";
                 ...
                 $pr_comment_type == $COMMENT_TYPE_OPTIONAL) {
            $pr_comment = htmlspecialchars($cRow['PrincipalComment'],
                                        ENT_QUOTES);
        }

        $termnum = $cRow['TermNumber'];
        $data = str_replace("&lt;&lt;average_t$termnum&gt;&gt;",
                            htmlspecialchars($term_average, ENT_QUOTES), $data);
        $data = str_replace("&lt;&lt;class_average_t$termnum&gt;&gt;",
                            htmlspecialchars($class_term_average, ENT_QUOTES),
                            $data);
        $data = str_replace("&lt;&lt;conduct_t$termnum&gt;&gt;",
                            htmlspecialchars($term_conduct, ENT_QUOTES), $data);
        $data = str_replace("&lt;&lt;class_conduct_t$termnum&gt;&gt;",
                            htmlspecialchars($class_term_conduct, ENT_QUOTES),
                            $data);
        $data = str_replace("&lt;&lt;rank_t$termnum&gt;&gt;",
                            htmlspecialchars($term_rank, ENT_QUOTES), $data);
        $data = str_replace("&lt;&lt;class_teacher_comment_t$termnum&gt;&gt;",
                            $ct_comment, $data);
        $data = str_replace(
                            "&lt;&lt;head_of_department_comment_t$termnum&gt;&gt;",
                            $hod_comment, $data);
        $data = str_replace("&lt;&lt;principal_comment_t$termnum&gt;&gt;",
                            $pr_comment, $data);
    }
    ...
    $data = str_replace("&lt;&lt;name&gt;&gt;",
                        htmlspecialchars($student_name, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;username&gt;&gt;",
                        htmlspecialchars($student_info['Username'], ENT_QUOTES),
                        $data);
    $data = str_replace("&lt;&lt;term&gt;&gt;",
                        htmlspecialchars($student_info['TermName'], ENT_QUOTES),
                        $data);
    $data = str_replace("&lt;&lt;year&gt;&gt;",
                        htmlspecialchars($student_info['Year'], ENT_QUOTES),
                        $data);
    ...
    $data = str_replace("&lt;&lt;class_count&gt;&gt;",
                        htmlspecialchars($class_count, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;average&gt;&gt;",
                        htmlspecialchars($average, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;class_average&gt;&gt;",
                        htmlspecialchars($class_average, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;overall_average&gt;&gt;",
                        htmlspecialchars($ovl_average, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;overall_rank&gt;&gt;",
                        htmlspecialchars($ovl_rank, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;class_overall_average&gt;&gt;",
                        htmlspecialchars($cls_ovl_average, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;overall_conduct&gt;&gt;",
                        htmlspecialchars($ovl_conduct, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;class_overall_conduct&gt;&gt;",
                        htmlspecialchars($cls_ovl_conduct, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;rank&gt;&gt;",
                        htmlspecialchars($rank, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;conduct&gt;&gt;",
                        htmlspecialchars($conduct, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;class_conduct&gt;&gt;",
                        htmlspecialchars($class_conduct, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;effort&gt;&gt;",
                        htmlspecialchars($effort, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;class_effort&gt;&gt;",
                        htmlspecialchars($class_effort, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;absences&gt;&gt;",
                        htmlspecialchars($absences, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;late&gt;&gt;",
                        htmlspecialchars($late, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;class_teacher&gt;&gt;",
                        htmlspecialchars($ct_name, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;head_of_department&gt;&gt;",
                        htmlspecialchars($hod_name, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;principal&gt;&gt;",
                        htmlspecialchars($pr_name, ENT_QUOTES), $data);
    $data = str_replace("&lt;&lt;class_teacher_comment&gt;&gt;", $ct_comment,
                        $data);
    $data = str_replace("&lt;&lt;head_of_department_comment&gt;&gt;",
                        $hod_comment, $data);
    ...
    // If we find a reference to t1, then include all subjects student was in this year
    $multiterm = strpos($data, "&lt;&lt;subject_average_t1&gt;&gt;");
    if ($multiterm === false) {
        $multiterm = strpos($data, "&lt;&lt;subject_mark_t1&gt;&gt;");
    }
    if ($multiterm === false) {
        $multiterm = false;
    } else {
    ...
    while ( $pos != false ) {
        ...
        $endpos = $pos + strpos(substr($data, $pos), "</table:table-row>") +
        ...
        if ($multiterm) {
            $query = "SELECT subject.Name AS SubjectName, subject.ShortName, subject.SubjectIndex, " .
                     ...
                     "       FROM subjecttype, class, classterm, " .
                     ...
                     "       AND   term.DepartmentIndex         = $depindex " .
                     "       AND   currentterm.TermIndex        = $termindex " .
                     "       AND   subject.YearIndex            = $yearindex " .
                     "       AND   subject.ShowInList           = 1 " .
                     "       GROUP BY subject.Name) AS tempsubjectlist, " .
                     "       subject LEFT OUTER JOIN subjectstudent " .
                     "            ON  subject.TermIndex = $termindex " .
                     "            AND subject.YearIndex = $yearindex " .
                     "            AND subjectstudent.SubjectIndex = subject.SubjectIndex " .
                     "            AND subjectstudent.Username = '{$student_info['Username']}' " .
                     "       LEFT OUTER JOIN nonmark_index AS average_index ON " .
                     "            subjectstudent.Average = average_index.NonmarkIndex " .
                     "           AND subjectstudent.Average IS NOT NULL " .
                     "       LEFT OUTER JOIN nonmark_index AS effort_index ON " .
        ...
        while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
            ...
            $reprow = str_replace("&lt;&lt;subject_name&gt;&gt;",
                                htmlspecialchars($row['SubjectName'],
                                                ENT_QUOTES), $data_row);
            $reprow = str_replace("&lt;&lt;subject_shortname&gt;&gt;",
                                htmlspecialchars($row['ShortName'], ENT_QUOTES),
                                $reprow);
            $reprow = str_replace("&lt;&lt;subject_type&gt;&gt;",
                                htmlspecialchars($row['SubjectType'],
                                                ENT_QUOTES), $reprow);
            $reprow = str_replace("&lt;&lt;subject_strippedname&gt;&gt;",
                                htmlspecialchars($stripped_name, ENT_QUOTES),
                                $reprow);
            $reprow = str_replace("&lt;&lt;subject_average&gt;&gt;",
                                htmlspecialchars($subject_average, ENT_QUOTES),
                                $reprow);
            $reprow = str_replace("&lt;&lt;subject_weight&gt;&gt;",
                                htmlspecialchars($weight, ENT_QUOTES), $reprow);
            $reprow = str_replace("&lt;&lt;subject_mark&gt;&gt;",
                                htmlspecialchars($average, ENT_QUOTES), $reprow);
            $reprow = str_replace("&lt;&lt;subject_effort&gt;&gt;",
                                htmlspecialchars($effort, ENT_QUOTES), $reprow);
            $reprow = str_replace("&lt;&lt;subject_conduct&gt;&gt;",
                                htmlspecialchars($conduct, ENT_QUOTES), $reprow);
            $reprow = str_replace("&lt;&lt;subject_comment&gt;&gt;", $comment,
                                $reprow);
            ...
            while ( $nrow = & $nres->fetchRow(DB_FETCHMODE_ASSOC) ) {
                ...
                } elseif ($nrow['AverageType'] == $AVG_TYPE_INDEX or
                         $nrow['AverageType'] == $AVG_TYPE_GRADE) {
                ...
                if ($nrow['CommentType'] == $COMMENT_TYPE_NONE) {
                    ...
                    if (! is_null($nrow['Comment'])) {
                        $comment = htmlspecialchars($nrow['Comment'],
                                                    ENT_QUOTES);
                    } else {
                        $comment = "";
                    }
                ...
                $reprow = str_replace(
                                    "&lt;&lt;subject_average_t$termnum&gt;&gt;",
                                    htmlspecialchars($subject_average,
                                                    ENT_QUOTES), $reprow);
                $reprow = str_replace("&lt;&lt;subject_mark_t$termnum&gt;&gt;",
                                    htmlspecialchars($average, ENT_QUOTES),
                                    $reprow);
                $reprow = str_replace(
                                    "&lt;&lt;subject_effort_t$termnum&gt;&gt;",
                                    htmlspecialchars($effort, ENT_QUOTES),
                                    $reprow);
                $reprow = str_replace(
                                    "&lt;&lt;subject_conduct_t$termnum&gt;&gt;",
                                    htmlspecialchars($conduct, ENT_QUOTES),
                                    $reprow);
                $reprow = str_replace(
                                    "&lt;&lt;subject_comment_t$termnum&gt;&gt;",
                                    $comment, $reprow);
            }
            ...
            $reprow = str_replace("&lt;&lt;overall_subject_average&gt;&gt;",
                                htmlspecialchars($subj_average, ENT_QUOTES),
                                $reprow);
            $reprow = str_replace("&lt;&lt;overall_subject_mark&gt;&gt;",
                                htmlspecialchars($std_average, ENT_QUOTES),
                                $reprow);
...
$output = array();
$output = exec("cd $tempdir; /usr/bin/zip -DXr $tempdir.odt *", $output,
            $retval);
unset($output);
