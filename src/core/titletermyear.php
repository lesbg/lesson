...
if (! isset($nochangeyt))
    $nochangeyt = false;
if (! isset($nochangeyear))
    $nochangeyear = false;
if (! isset($nochangeterm))
    $nochangeterm = false;
if (! isset($showterm))
    $showterm = true;
if (! isset($showyear))
...
foreach ( $_GET as $tty_getkey => $getval ) {
    if ($tty_getkey != 'year' && $tty_getkey != 'term' &&
         $tty_getkey != 'dept' && $tty_getkey != 'location')
        $tty_link .= "&amp;$tty_getkey=$getval";
}
if ($showyear) {
    $tty_res = &   $db->query(
                        "SELECT Year, YearIndex FROM year " . // Run query to get year
                         "ORDER BY YearNumber");
    ...
    if ($nochangeyt == false and $nochangeyear == false) {
        if (! is_null($tty_oldyear)) {
            $tty_nlink = $tty_link . "&amp;year=" .
                 dbfuncString2Int($tty_oldyear);
            $tty_prevbutton = dbfuncGetButton($tty_nlink, "<", "small",
                                            "prevnext", "Previous school year");
        } else {
            $tty_prevbutton = dbfuncGetDisabledButton("<", "small", "prevnext");
        }
        if (! is_null($tty_newyear)) {
            $tty_nlink = $tty_link . "&amp;year=" .
                         dbfuncString2Int($tty_newyear);
            $tty_nextbutton = dbfuncGetButton($tty_nlink, ">", "small",
                                            "prevnext", "Next school year");
        } else {
...
if ($nochangeyt == false and isset($depindex) and ! is_null($depindex) and
     $showdeps) {
    if (! $showalldeps) {
        if (! $admin_page) {
    ...
    /*                      "UNION " .
        ...
        $query = "(SELECT hod.DepartmentIndex, department.Department FROM " .
...
if ($tty_res->numRows() > 1) {
    ...
    while ( $tty_row = & $tty_res->fetchRow(DB_FETCHMODE_ASSOC) ) {
        ...
        $tty_nlink = $tty_link . "&amp;dept=" .
                     dbfuncString2Int($tty_row['DepartmentIndex']);
        $tty_button = dbfuncGetButton($tty_nlink, $tty_row['Department'],
                                    "small", $buttontype,
                                    "{$tty_row['Department']} department");
        echo "$tty_button ";
    }
    echo "</i></h3>\n";
}
...
if ($nochangeyt == false and $nochangeterm == false) {
    if (! is_null($tty_oldterm)) {
        ...
        $tty_prevbutton = dbfuncGetButton($tty_nlink, "<", "small", "prevnext",
                                        "Previous term");
    } else {
        $tty_prevbutton = dbfuncGetDisabledButton("<", "small", "prevnext");
    }
    if (! is_null($tty_newterm)) {
        ...
        $tty_nextbutton = dbfuncGetButton($tty_nlink, ">", "small", "prevnext",
                                        "Next term");
    } else {
        $tty_nextbutton = dbfuncGetDisabledButton(">", "small", "prevnext");
    }
