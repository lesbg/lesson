...
if (dbfuncGetPermission($permissions, $PERM_ADMIN)) { // Make sure user has permission to view and
	...
	$nameAsc = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=0", "A", "small", "sort", 
							"Sort ascending");
	$nameDec = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=1", "D", "small", "sort", 
							"Sort descending");
	$stctAsc = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=2", "A", "small", "sort", 
							"Sort ascending");
	$stctDec = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=3", "D", "small", "sort", 
							"Sort descending");
	$asctAsc = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=4", "A", "small", "sort", 
							"Sort ascending");
	$asctDec = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=5", "D", "small", "sort", 
							"Sort descending");
	$avgAsc = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=6", "A", "small", "sort", 
							"Sort ascending");
	$avgDec = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=7", "D", "small", "sort", 
							"Sort descending");
	$perAsc = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=8", "A", "small", "sort", 
							"Sort ascending");
	$perDec = dbfuncGetButton(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php") .
							 "&amp;sort=9", "D", "small", "sort", 
							"Sort descending");
	...
	$query = "SELECT query.CanDoReport, query.SubjectIndex, query.Average, query.ClassName, " .
			 ...
			 "       MIN(subjectstudent.ReportDone) AS ReportDone " . "FROM ( " .
			 ...
			 "       GROUP BY subject.SubjectIndex) " . " ) AS query " .
			 ...
			 "GROUP BY query.SubjectIndex " . "ORDER BY $sortorder";
	$res = &  $db->query($query);
	if (DB::isError($res))
		die($res->getDebugInfo()); // Check for errors in query
	
	$newlink = "index.php?location=" .
			 dbfuncString2Int("admin/subject/new.php") . // link to create a new subject
"&amp;next=" .
			 dbfuncString2Int(
							"index.php?location=" .
							 dbfuncString2Int("admin/subject/list.php"));
	$newbutton = dbfuncGetButton($newlink, "New subject", "medium", "", 
								"Create new subject");
	...
	if ($res->numRows() > 0) {
		...
		while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
			...
			$editbutton = dbfuncGetButton($editlink, "E", "small", "edit", 
										"Add or remove students from subject");
			$ttbutton = dbfuncGetButton($ttlink, "T", "small", "tt", 
										"View subject's timetable");
			if ($row['CanDoReport']) {
				if ($row['ReportDone']) {
					$repbutton = dbfuncGetButton($replink, "V", "small", 
												"report", "View report");
				} else {
					$repbutton = dbfuncGetButton($replink, "R", "small", 
												"report", "Edit report");
				}
			} else {
				$repbutton = "";
			...
			$tResult = & $db->query(
								"SELECT user.Username, user.FirstName, user.Surname FROM user, " .
								 "       subjectteacher " .
								 "WHERE subjectteacher.SubjectIndex = {$row['SubjectIndex']} " .
								 "AND   user.Username = subjectteacher.Username " .
								 "ORDER BY user.Username");
			if (DB::isError($tResult))
				die($tResult->getDebugInfo()); // Check for errors in query
