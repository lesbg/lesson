...
/**
 * ***************************************************************
 * admin/statistics/class.php (c) 2011, 2016 Jonathan Dieter
 *
...
if (isset($_GET['cumulative']) and
     safe(dbfuncInt2String($_GET['cumulative'])) == "1") {
    $cumulative = true;
}
...
if (! $is_ct and ! $is_hod and ! $is_principal and ! $is_admin) {
    ...
    log_event($LOG_LEVEL_ERROR, "admin/statistics/class.php",
            $LOG_DENIED_ACCESS, "Tried to get statistics for $class.");
...
if ($cumulative and
     ($average_type == $CLASS_AVG_TYPE_PERCENT or
...
$query .= "AND   subject.YearIndex = $yearindex " .
         ...
         "AND   subjectstudent.Average != -1 " . "GROUP BY subject.Name " .
...
$cumlink = "";
if (! $cumulative) {
$cumlink = "index.php?location=" .
         dbfuncString2Int("admin/statistics/class.php") . // link to create a new subject
"&amp;key=" . $_GET['key'] . "&amp;keyname=" . $_GET['keyname'] .
         "&amp;cumulative=" . dbfuncString2Int("1");
}
$indlink = "";
if ($cumulative) {
$indlink = "index.php?location=" .
         dbfuncString2Int("admin/statistics/class.php") . // link to create a new subject
"&amp;key=" . $_GET['key'] . "&amp;keyname=" . $_GET['keyname'] .
...
$indbutton = dbfuncGetButton($indlink, "Individual", "medium", "",
                            "Show each term's marks by themselves");
} else {
$cumbutton = dbfuncGetButton($cumlink, "Cumulative", "medium", "",
                            "Show cumulative averages for each student");
...
if ($row['Username'] != $prev_username) {
    ...
    $detaillink = "index.php?location=" .
                 dbfuncString2Int("teacher/report/class_modify.php") . // link to create a new subject
                 "&amp;key=" . $_GET['key'] . "&amp;keyname=" . $_GET['keyname'] .
                 "&amp;keyname2=" .
    ...
    if (! $cumulative) {
        ...
        $query = "SELECT ROUND(AVG(ROUND(classlist.Average))) AS ClassAverage, " .
                 "       1 AS Type " .
                 "       FROM classlist, classterm, class, term, " .
                 "            term AS selected_term " .
                 "WHERE selected_term.TermIndex        = $termindex " .
                 "AND   term.TermNumber               <= selected_term.TermNumber " .
                 "AND   term.DepartmentIndex           = selected_term.DepartmentIndex " .
