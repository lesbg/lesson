...
if ($res->numRows() == 0 and ! $is_admin) {
	...
	log_event($LOG_LEVEL_ERROR, "admin/attendance/first_period.php", 
...
$query = "(SELECT subject.Name, NULL AS ClassName, subject.Grade, period.PeriodIndex, " .
		 ...
		 "        subject " . " WHERE  subject.ClassIndex          IS NULL " .
		 ...
		 " AND    period.PeriodIndex          = timetable.PeriodIndex " .
		 " AND    period.Period               = 1 " .
		 " GROUP BY subject.SubjectIndex) " . "ORDER BY Grade, ClassName, Name";
$res = &  $db->query($query);
if (DB::isError($res))
	die($res->getDebugInfo()); // Check for errors in query
...
while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
	...
	$link = "index.php?location=" .
			 ...
			 dbfuncString2Int($row['Name']) . "&amp;next=" . dbfuncString2Int(
																			"index.php?location=" . dbfuncString2Int(
																													"admin/attendance/first_period.php") .
																			 "&amp;key=" .
																			 dbfuncString2Int(
																							$date));
