		...
		if($conductRow =& $conductRes->fetchrow(DB_FETCHMODE_ASSOC) and $conductRow['Conduct'] != "") {
			...
			if($res->numRows() > 0) {
				while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
					...
					if((!is_null($row['ServedType']) and $row['ServedType'] == 1) or $row['DoPunishment'] == 0) {
						$alt = " class=\"$alt_step\"";
					} elseif((!is_null($row['ServedType']) and $row['ServedType'] == 0) or (is_null($row['ServedType']) and !is_null($row['PunishDate'])) or $row['DisciplineWeight'] == 0) {
					...
					// Write beginning of term if this was the last mark for the term.
					if($conduct_mark == 100 and $row['TermIndex'] == $termindex and !$print_beginning) {
						$alt_count += 1;
						if($alt_count % 2 == 0) {
							$alt_step = "alt";
						} else {
