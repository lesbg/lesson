<?php
	...
	$query =	"SELECT Title, SubjectIndex, YearIndex, TermIndex, Score, Uploadable FROM view_marks " .
	...
	if($is_admin or ($studentusername == $username and $res->numRows() > 0)) {
		...
		if($row['Uploadable'] == 1 and (is_null($row['Score']) or $row['Score'] == $MARK_LATE)) {
			$link   = "index.php?location=" .  dbfuncString2Int("student/doupload.php") .
							"&amp;key=" .            dbfuncString2Int($assignmentindex) .
