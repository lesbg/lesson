...
if ($is_admin or ($studentusername == $username and $res->numRows() > 0)) {
	...
	if ($row['Uploadable'] == 1 and
		...
		$tres = &  $db->query(
							"SELECT subjectteacher.Username, subject.RenameUploads " .
		...
		if (DB::isError($tres))
			die($tres->getDebugInfo()); // Check for errors in query
		if ($trow = & $tres->fetchRow(DB_FETCHMODE_ASSOC)) {
			...
			if (move_uploaded_file($_FILES['hw']['tmp_name'], "$dst_dir/$file")) {
				while ( $trow = & $tres->fetchRow(DB_FETCHMODE_ASSOC) ) {
					$new_dir = & dbfuncGetDir($assignmentindex, 
											$row['UploadName'], 
											$trow['Username']);
				...
				log_event($LOG_LEVEL_TEACHER, "student/doupload.php", 
						$LOG_ASSIGNMENT, 
