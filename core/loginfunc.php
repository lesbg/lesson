...
function &loginfuncDoLogin() {
	...
	$params = array(
			"dsn" => $DSN, // Global variable set in index.php
			"table" => "user",
	...
	if (isset($_SERVER['REMOTE_HOST']) and
		 strtolower(
					substr($_SERVER['REMOTE_HOST'], - strlen($LOCAL_HOSTS))) ==
			...
			if ($username != "") {
				...
				if ($authSession->checkAuth() == FALSE) {
					if ($authSession->getStatus() == AUTH_WRONG_LOGIN) {
						...
						log_event($LOG_LEVEL_ACCESS, "core/loginfunc.php", 
								$LOG_ERROR, 
								"Failed login (Incorrect password or username).", 
								0);
						/*
