...
if ($is_principal or $is_hod or $is_counselor or $is_classteacher or
	 $is_supportteacher or $is_teacher or $prev_cn) {
	...
	$query = "SELECT user.Title, user.FirstName, user.Surname " .
			 "       FROM principal, user " . "WHERE Level = 1 " .
	...
	if ($writable) {
		...
		$addbutton = dbfuncGetButton($addLink, "New casenote", "medium", "", 
									"Create new casenote for $studentfirstname");
		if (! $is_on_wl) {
			if ($is_counselor) {
				...
				$wlbutton = dbfuncGetButton($wlnLink, "Add to my watchlist", 
											"medium", "cn", 
											"Add $studentfirstname to my casenote watchlist");
			} else {
				$res = &  $db->query(
								"DELETE FROM casenotewatch " .
								 "WHERE WorkerUsername='{$username}'");
			...
			if ($is_counselor) {
				...
				$wlbutton = dbfuncGetButton($wlnLink, 
											...
											"delete", 
											"Remove $studentfirstname from my casenote watchlist");
			} else {
				$wlbutton = "";
			}
		}
	...
	while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
		...
		if ($row['Level'] == 0) {
			...
			if ($row['Level'] == 1) {
				$text = array(
						"all $studentfirstname's teachers",
						"all counselors",
						$author
				);
