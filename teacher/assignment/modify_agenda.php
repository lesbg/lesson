...
if ($res->numRows() > 0 or $is_admin) {
    ...
    if (isset($aRow['DescriptionFileType']) and
         $aRow['DescriptionFileType'] != "") {
        $descrtype0 = "";
        $descrtype1 = "checked";
    } else {
        $descrtype0 = "checked";
    ...
    log_event($LOG_LEVEL_EVERYTHING, "teacher/assignment/modify_agenda.php",
            $LOG_TEACHER, "Viewed agenda item ($title) for {$aRow['Name']}.");
    ...
    if (DB::isError($asr))
        die($asr->getDebugInfo()); // Check for errors in query
    $aRow = & $asr->fetchRow(DB_FETCHMODE_ASSOC);
    log_event($LOG_LEVEL_ERROR, "teacher/assignment/modify_agenda.php",
            $LOG_DENIED_ACCESS,
