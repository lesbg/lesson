...
if ($is_teacher or $is_support_class_teacher or $is_admin) {
    ...
    $asmntlink = "index.php?location=" .
                 dbfuncString2Int("teacher/assignment/list.php") . "&amp;key=" .
                 dbfuncString2Int($subjectindex) . "&amp;keyname=" .
                 $_GET['keyname'];
    $newlink = "index.php?location=" .
             dbfuncString2Int("teacher/assignment/new_agenda.php") . "&amp;key=" .
             dbfuncString2Int($subjectindex) . "&amp;keyname=" .
             $_GET['keyname'];

    $asmntbutton = dbfuncGetButton($asmntlink, "Assignments", "medium", "",
                                "List assignments for this subject");
    if ($can_modify == 1) {
    ...
    if ($res->numRows() > 0) {
        ...
        while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
            ...
            $modifylink = "index.php?location=" .
                         dbfuncString2Int(
                                        "teacher/assignment/modify_agenda.php") .
                         "&amp;key=" .
    ...
    log_event($LOG_LEVEL_EVERYTHING, "teacher/assignment/list_agenda.php",
            $LOG_STUDENT,
            "Viewed all of " . dbfuncInt2String($_GET['keyname']) .
             "'s agenda items.");
} else {
    /* Log unauthorized access attempt */
    log_event($LOG_LEVEL_ERROR, "teacher/assignment/list_agenda.php",
            $LOG_DENIED_ACCESS,
            "Tried to access " . dbfuncInt2String($_GET['keyname']) .
             "'s agenda items.");
