<?php
	...
	if($res->numRows() > 0 || dbfuncGetPermission($permissions, $PERM_ADMIN)) {
		...
		/* Get student list */
		$res =&  $db->query("SELECT user.FirstName, user.Surname, user.Username, query.ClassOrder FROM user, " .
		...
		log_event($LOG_LEVEL_EVERYTHING, "teacher/assignment/print_gradesheet.php", $LOG_TEACHER,
				"Accessed printable gradesheet for $title.");
	} else {  // User isn't authorized to view or change scores.
		/* Log unauthorized access attempt */
		log_event($LOG_LEVEL_ERROR, "teacher/assignment/print_gradesheet.php", $LOG_DENIED_ACCESS,
