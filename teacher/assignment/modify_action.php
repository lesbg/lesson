...
/**
 * ***************************************************************
 * teacher/assignment/modify_action.php (c) 2004-2007, 2016 Jonathan Dieter
 *
...
if ($res->numRows() > 0 or $is_admin) {
    ...
    if (($average_type == $AVG_TYPE_PERCENT or $average_type == $AVG_TYPE_GRADE) and
        ...
        $res = &  $db->query(
                        "SELECT categorylist.CategoryListIndex FROM assignment, category, " .
                         "       categorylist " .
                         "WHERE assignment.AssignmentIndex = $assignmentindex " .
    ...
    if ($_POST['uploadable'] == 1) {
        ...
        $res = & $db->query(
                        "SELECT UploadName, Uploadable FROM assignment WHERE AssignmentIndex = $assignmentindex");
        if (DB::isError($res))
            die($res->getDebugInfo()); // Check for errors in query
        if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
            if ($row['Uploadable'] != 0) {
                if ($row['UploadName'] != $upload_name) {
                    dbfuncMoveDir($assignmentindex, $row['UploadName'],
                                $upload_name);
                }
            }
        }
...
if ($_POST['action'] == "Convert to assignment" and
     $average_type != $AVG_TYPE_NONE) {
...
if ($agenda == "0") {
$res = & $db->query(
                "SELECT subjectstudent.Username FROM " . // Get list of students
...
log_event($LOG_LEVEL_TEACHER, "teacher/assignment/modify_action.php",
        $LOG_TEACHER, "Modified assignment ($title) for {$aRow['Name']}.");
...
log_event($LOG_LEVEL_ERROR, "teacher/assignment/modify_action.php",
        $LOG_DENIED_ACCESS, "Tried to modify marks for {$aRow['Name']}.");
