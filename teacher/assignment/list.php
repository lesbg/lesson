...
if ($is_teacher or $is_support_class_teacher or $is_admin) {
    ...
    $newlink = "index.php?location=" .
             dbfuncString2Int("teacher/assignment/new.php") . "&amp;key=" .
             dbfuncString2Int($subjectindex) . "&amp;keyname=" .
             $_GET['keyname'];
    $agendalink = "index.php?location=" .
                 dbfuncString2Int("teacher/assignment/list_agenda.php") .
    ...
    $agendabutton = dbfuncGetButton($agendalink, "Agenda items", "medium", "",
                                    "List agenda items for this subject");
    ...
    if ($average_type != 0) {
        ...
        $mres = & $db->query($query);
        if (DB::isError($mres))
            die($mres->getDebugInfo()); // Check for errors in query
