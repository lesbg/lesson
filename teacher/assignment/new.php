<?php
	...
	if($res->numRows() > 0 or $is_admin) {
		...
		if($average_type == $AVG_TYPE_INDEX and !is_null($average_type_index)) {
			...
			$sres =&  $db->query($query);
			if(DB::isError($sres)) die($res->getDebugInfo());           // Check for errors in query
	...
	//			echo "                     <option value='NULL' selected>(None)\n";
		...
		log_event($LOG_LEVEL_ERROR, "teacher/assignment/new.php", $LOG_DENIED_ACCESS, 
					"Tried to create new assignment for $subtitle.");
