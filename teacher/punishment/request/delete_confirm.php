<?php
	/*****************************************************************
	 * teacher/punishment/request/delete_confirm.php  (c) 2006 Jonathan Dieter
	 *
	...
	if($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
		...
		if($is_admin or $row['WorkerUsername']==$username) {
			$link     = "index.php?location=" . dbfuncString2Int("teacher/punishment/request/delete.php") .
						"&amp;key=" .           $_GET['key'] .
						"&amp;next=" .          $_GET['next'];
			...
			log_event($LOG_LEVEL_ERROR, "teacher/punishment/request/delete_confirm.php", $LOG_DENIED_ACCESS,
