<?php
	...
	if(dbfuncGetPermission($permissions, $PERM_ADMIN) or $perm >= $PUN_PERM_REQUEST) {
				...
				if($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
					...
					if($failed == 0) {
						if($_POST["action"] == "Save") {
							...
							if(DB::isError($res)) die($res->getDebugInfo()); // Check for errors in query
							echo " done</p>\n";
							log_event($LOG_LEVEL_TEACHER, "teacher/punishment/request/new_action.php", $LOG_TEACHER,
		...
		log_event($LOG_LEVEL_ERROR, "teacher/punishment/request/new_action.php", $LOG_DENIED_ACCESS,
