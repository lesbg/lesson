...
$link = "index.php?location=" .
		 dbfuncString2Int("teacher/conduct/modify_action.php") . "&amp;key=" .
		 $_GET['key'] . "&amp;next=" . dbfuncString2Int($backLink);
...
if ($res->numRows() > 0 or $is_admin) {
	$res = &  $db->query(
					"SELECT user.FirstName, user.Surname, user.Username, subjectstudent.Conduct " .
	...
	log_event($LOG_LEVEL_EVERYTHING, "teacher/assignment/modify.php", 
			$LOG_TEACHER, "Viewed assignment ($title) for {$aRow['Name']}.");
	...
	if (DB::isError($asr))
		die($asr->getDebugInfo()); // Check for errors in query
	$aRow = & $asr->fetchRow(DB_FETCHMODE_ASSOC);
	log_event($LOG_LEVEL_ERROR, "teacher/assignment/modify.php", 
			$LOG_DENIED_ACCESS, 
