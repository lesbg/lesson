<?php
	...
	if($is_admin or $is_class_teacher or $res->numRows() > 0) {
		...
		if($_POST["action"] == "Check $direction") {
			...
			if(!isset($_POST['state']) or is_null($_POST['state']) or $_POST['state'] == "NULL" or $_POST['state'] == "") {
			...
			if(!isset($_POST['student']) or is_null($_POST['student']) or $_POST['student'] == "") {
			...
			if($oldstate - $state > 2 and $comment == "NULL" and $book_status_type_index == 1) {
		...
		log_event($LOG_LEVEL_ERROR, "teacher/book/check_in_out_copy_action.php", $LOG_DENIED_ACCESS,
				"Attempted to check $direction a book.");
