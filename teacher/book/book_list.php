<?php
	...
	if($is_admin or $username == $checkuser) {
		if($is_class_teacher or $is_teacher) {
			if($is_class_teacher) {
				if($display_type != 0) { 
					$byclass_link =	"index.php?location=" .	dbfuncString2Int("teacher/book/book_list.php") .
									"&amp;key=" .           $_GET['key'] .
									"&amp;keyname=" .       $_GET['keyname'] .
								 	"&amp;key2=" .			dbfuncString2Int('0') .
								 	"&amp;key5=" .          $_GET['key5'];
					$byclass_button = dbfuncGetButton($byclass_link, "Group by class", "medium", "", "Show book list grouped by class");
				} else {
					$byclass_button = dbfuncGetDisabledButton("Group by class", "medium", "");
				}
			} else {
				$byclass_button = "";
				if($display_type == 0) {
			...
			if($display_type != 1) { 
				$bysubj_link =	"index.php?location=" .	dbfuncString2Int("teacher/book/book_list.php") .
								"&amp;key=" .           $_GET['key'] .
								"&amp;keyname=" .       $_GET['keyname'] .
			...
			if($display_type != 2) { 
				$bybook_link =	"index.php?location=" .	dbfuncString2Int("teacher/book/book_list.php") .
								"&amp;key=" .           $_GET['key'] .
								"&amp;keyname=" .       $_GET['keyname'] .
			...
			$bybook_button = dbfuncGetButton($bybook_link, "Group by book", "medium", "", "Show book list grouped by book");
			
			$bysubject_button = dbfuncGetButton($bysubj_link, "Group by subject", "medium", "", "Show book list grouped by subject");
		...
		if($display_type == 0) {
			...
			if(is_null($classindex)) {
				...
				if($res->numRows() == 1) {
					...
					while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
						...
						$link =	"index.php?location=" .  dbfuncString2Int("teacher/book/book_list.php") .
								"&amp;key=" .            $_GET['key'] .
								"&amp;keyname=" .        $_GET['keyname'] .
								"&amp;key2=" .           $_GET['key2'] .
								"&amp;key3=" .           dbfuncString2Int($row['ClassIndex']) .
								"&amp;key3name=" .       dbfuncString2Int($row['ClassName']) .
								"&amp;key5=" .          $_GET['key5'];
				...
				if($res->numRows() > 0) {
					...
					while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
						...
						$link =	"index.php?location=" .  dbfuncString2Int("teacher/book/book_list.php") .
								...
								"&amp;key3=" .           dbfuncString2Int($row['ClassIndex']) .
								"&amp;key3name=" .       dbfuncString2Int($row['ClassName']) .
								"&amp;key4=" .           dbfuncString2Int($row['Username']) .
								"&amp;key4name=" .       dbfuncString2Int("{$row['FirstName']} {$row['Surname']} ({$row['Username']})") .
								"&amp;key5=" .          $_GET['key5'];
						echo "         <tr$alt>\n";
						echo "            <td>$orderNum</td>\n";
				...
				if($res->numRows() == 1) {
					...
					while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
						...
						$link =	"index.php?location=" .  dbfuncString2Int("teacher/book/book_list.php") .
								...
								"&amp;keyname=" .        $_GET['keyname'] .
								"&amp;key2=" .           $_GET['key2'] .
								"&amp;key3=" .           dbfuncString2Int($row['SubjectIndex']) .
								"&amp;key3name=" .       dbfuncString2Int($row['Name']) .
								"&amp;key5=" .          $_GET['key5'];
			...
			$query =	"SELECT book_title.BookTitle, book_title.BookTitleIndex, class.ClassIndex, class.ClassName, class.Grade " .
						...
						"   LEFT OUTER JOIN " .
						"    (currentterm INNER JOIN subject " .
		...
		if(($display_type == 0 and !is_null($classindex) and !is_null($student_username)) or 
		   ($display_type == 1 and !is_null($subjectindex) and !is_null($classindex) and !is_null($student_username))) {
			...
			if($res->numRows() > 0) {
				$back_link =	"index.php?location=" . dbfuncString2Int("teacher/book/book_list.php") .
								"&amp;key=" .            $_GET['key'] .
								"&amp;keyname=" .        $_GET['keyname'] .
								"&amp;key2=" .           $_GET['key2'] .
								"&amp;key3=" .           $_GET['key3'] .
								...
								"&amp;key5=" .           $_GET['key5'];
				$back_button =	dbfuncGetButton($back_link,    "⇠", "large", "view", "Back to class list");
				
				echo "      <h2 align='center'>$back_button $subheader</h2>\n";
				$show_link =	"index.php?location=" . dbfuncString2Int("teacher/book/book_list.php") .
								"&amp;key=" .            $_GET['key'] .
								"&amp;keyname=" .        $_GET['keyname'] .
								"&amp;key2=" .           $_GET['key2'] .
								"&amp;key3=" .           $_GET['key3'] .
				...
				if($show_all_books == 1) {
					...
					$show_thisclass_button = dbfuncGetButton($show_link, "Show class", "medium", "", "Show this class's books");
				} else {
					$show_link .= "&amp;key5=" .           dbfuncString2Int('1');
					
					$show_all_button = dbfuncGetButton($show_link, "Show all", "medium", "", "Show all books");
					$show_thisclass_button = dbfuncGetDisabledButton("Show class", "medium", "");
				}
				...
				while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
					...
					if($row['BookTitleIndex'] == $prev_code and is_null($row['Date']))
						continue;
					...
					$link =		"index.php?location=" .	dbfuncString2Int("teacher/book/check_in_out_copy.php") .
								"&amp;key4=" .			dbfuncString2Int($student_username) .
								"&amp;next=" .			dbfuncString2Int(
									"index.php?location=" . dbfuncString2Int("teacher/book/book_list.php") .
									"&amp;key=" .            $_GET['key'] .
									"&amp;keyname=" .        $_GET['keyname'] .
									"&amp;key2=" .           $_GET['key2'] .
									"&amp;key3=" .           $_GET['key3'] .
					...
					if($row['BookStatusTypeIndex'] == 2) {
						$link .=	"&amp;key=" .           dbfuncString2Int($row['BookIndex']) .
									"&amp;keyname=" .       dbfuncString2Int($row['BookTitle'] . " copy #" . $row['BookNumber']) .	
									"&amp;key2=" . 			dbfuncString2Int(1);
						...
						if(!is_null($row['BookIndex'])) {
							...
							for($i=0; $i<count($bookindex); $i++) {
								if(isset($bookindex[$i]) and isset($booknumber[$i])) {
									echo "                     <option value='{$bookindex[$i]}'>{$booknumber[$i]}</option>\n";
								}	
							}
							echo "                  </select>\n";
						} else {
							$link =		"index.php?location=" .	dbfuncString2Int("teacher/book/choose_book_number.php") .
