...
if (! isset($_GET['key'])) {
    ...
    $checkuser = dbfuncInt2String($_GET['key']);
}
...
if (isset($_GET['key3']) and ! is_null($_GET['key3']) and $_GET['key3'] != "") {
    ...
    $subheader = dbfuncInt2String($_GET['key3name']);
    $subject_index = $class_index;
} else {
    $_GET['key3'] = "";
    $class_index = NULL;
    $class_name = NULL;
...
if (isset($_GET['key4']) and ! is_null($_GET['key4']) and $_GET['key4'] != "") {
    $student_username = dbfuncInt2String($_GET['key4']);
    $subheader = "Books for " . dbfuncInt2String($_GET['key4name']);
} else {
    $_GET['key4'] = "";
...
if ($is_admin or $username == $checkuser) {
    ...
    if ($display_type == 0) {
        ...
        if (! is_null($class_index)) {
            $query = $pdb->prepare(
                "SELECT class.ClassIndex, class.ClassName, class.Grade " .
                "  FROM class " .
                "WHERE class.ClassTeacherUsername = :checkuser " .
                "AND   class.ClassIndex = :classindex " .
                "AND   class.YearIndex = :yearindex "
            );
            $query->execute(['checkuser' => $checkuser,
        ...
        if (is_null($class_index)) {
            $query = $pdb->prepare(
                "SELECT class.ClassIndex, class.ClassName, class.Grade, COUNT(classlist.ClassListIndex) AS StudentCount " .
        ...
        if (! is_null($class_index) and is_null($student_username)) {
            $query = $pdb->prepare(
                "SELECT user.FirstName, user.Surname, user.Username, newmem.Username AS New, specialmem.Username AS Special, " .
        ...
        if (is_null($subject_index)) {
            $query = $pdb->prepare(
                "SELECT subject.Name, COUNT(subjectstudent.SubjectStudentIndex) AS StudentCount, subject.SubjectIndex " .
        ...
        $subquery = $pdb->prepare(
            "SELECT MyBookTitle.BookTitleIndex, book.BookIndex, book_status.BookStatusIndex, book_state.BookStateIndex FROM " .
            "       book INNER JOIN " .
        ...
        $subquery->execute(['class_index' => $class_index,
                            'student_username' => $student_username]);
        $query = NULL;
    } elseif ($display_type == 2) {
        $query = $pdb->prepare(
            "SELECT book_title.BookTitle, book_title.BookTitleIndex, class.ClassIndex, class.ClassName, class.Grade " .
    ...
    if (($display_type == 0 and ! is_null($class_index) and
         ! is_null($student_username)) or
         ($display_type == 1 and ! is_null($subject_index) and
         ! is_null($class_index) and ! is_null($student_username))) {
        ...
        if ($display_type == 0) {
            ...
            $query = "SELECT BookIndex, BookNumber, BookTitleIndex, BookTitle, SubjectType, BookStatusTypeIndex, `Date`, BookState, \n" .
