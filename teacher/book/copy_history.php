...
/**
 * ***************************************************************
 * teacher/book/copy_history.php (c) 2010, 2018 Jonathan Dieter
 *
...
$query = $pdb->prepare(
    "SELECT book.PurchaseDate, book_status.BookStatusTypeIndex, book_status_type.BookStatusType, " .
...
if($data) {
    ...
    foreach($data as $row) {
        ...
        if ($row['BookStatusTypeIndex'] == 1) {
            ...
            if ($in_username == "") {
                $nrow = $pdb->query(
                    "SELECT BookStatusType FROM book_status_type WHERE BookStatusTypeIndex=1"
                )->fetch();
                if ($nrow)
    ...
    if ($old_status != 3) {
        ...
        $nquery = $pdb->prepare(
            "SELECT PurchaseDate FROM book WHERE BookIndex=:book_index"
        );
        $nquery->execute(['book_index' => $book_index]);
        $nrow = $nquery->fetch();
        if ($nrow)
            $book_status_type = $nrow['PurchaseDate'];
        else
            $book_status_type = NULL;
