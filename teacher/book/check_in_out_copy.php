<?php
	...
	if(!isset($_GET['keyname'])) {
		...
		if ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
			$_GET['keyname'] = dbfuncString2Int($row['BookTitle'] . " copy #" . $row['BookNumber']);
		} else {
	...
	if($is_admin or $is_class_teacher or $res->numRows() > 0) {
		...
		$link  =	"index.php?location=" . dbfuncString2Int("teacher/book/check_in_out_copy_action.php") .
					"&amp;key=" .			$_GET['key'] .
		...
		if($ok) {
			...
			if(!isset($_POST['state']) or is_null($_POST['state']) or $_POST['state'] == "") {
				if($book_status_type_index == 1) {
					$_POST['state'] = NULL;
				} else {
			...
			if(!is_null($_POST['student']) and (!isset($_POST['class']) or is_null($_POST['class']))) {
			...
			if (!isset($_POST['class']) or is_null($_POST['class']) or $_POST['class'] == "") {
			...
			if ($_POST['class'] != "") {
				if($_POST['class'] == "teacher") {
					...
					while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
						if(isset($_POST['student']) and $_POST['student'] == $row['Username']) {
							$default = "selected";
						} else {
							$default = "";
						}
		...
		log_event($LOG_LEVEL_EVERYTHING, "teacher/book/check_in_out_copy.php", $LOG_ADMIN,
				"Checked $direction copy of $title.");
	} else {
		/* Log unauthorized access attempt */
		log_event($LOG_LEVEL_ERROR, "teacher/book/check_in_out_copy.php", $LOG_DENIED_ACCESS,
				"Attempted to check $direction copy of $title.");
