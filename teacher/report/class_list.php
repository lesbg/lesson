...
if ($is_admin or $is_principal) {
	$link = "index.php?location=" . dbfuncString2Int("report/show_class.php") .
		 "&amp;key=" . dbfuncString2Int($classtermindex) . "&amp;key3=" .
...
while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
	...
	$link = "index.php?location=" .
			 ...
			 dbfuncString2Int($classtermindex) . "&amp;key2=" .
	...
	if ($is_admin or $is_principal) {
		$link = "index.php?location=" . dbfuncString2Int("report/show.php") .
			 "&amp;key=" . dbfuncString2Int($classtermindex) . "&amp;key2=" .
...
if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
	if ($is_ct and ! $row['CTCommentDone'])
		$show_finish = true;
}
...
if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
	if ($is_hod and ! $row['HODCommentDone'])
		$show_finish = true;
}
...
if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
	if ($is_principal and ! $row['PrincipalCommentDone'])
		$show_finish = true;
}
...
if ($row = & $res->fetchRow(DB_FETCHMODE_ASSOC)) {
	if (($is_principal or $is_hod or $is_admin) and ! $row['ReportDone'])
		$show_rpt_close = true;
}
...
if ($show_finish or $is_hod or $is_admin or $is_principal) {
	$link = "index.php?location=" .
		 ...
		 dbfuncString2Int($classtermindex) . "&amp;keyname=" . $_GET['keyname'];
