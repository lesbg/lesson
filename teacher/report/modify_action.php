<?php
	...
	foreach($_POST AS $postkey => $postval) {
		if(substr($postkey, 0, 13) == "find_comment_" and $postval == "...") {
			...
			if(!isset($_POST['student_username'])) continue;
			...
			if(isset($_POST["cval_{$st_username}"]) and !is_null($_POST["cval_{$st_username}"]) and $_POST["cval_{$st_username}"] != "" and strlen($postval) > 10) {
				$cval_count += 1;
			...
			while($startloc !== false) {
				...
				$cval_count += 1;
				$cval_total += $strength;
				$_POST["cval_{$st_username}"] = strval(floatval($cval_total) / floatval($cval_count));
				$startloc = strpos($postval, '{');
			}
			$postval = str_replace('}', ')', $postval);
	...
	if($_POST['action'] == "Update" or $_POST['action'] == "Apply conduct to all my subjects" or $_POST['action'] == "I'm finished with these marks") {
		...
		if($_POST['action'] == "Update" or $_POST['action'] == "Apply conduct to all my subjects") {
...
/*					if($effort_type == $EFFORT_TYPE_PERCENT) {
				...
				$conduct = "-1";
				if($conduct_type != $CONDUCT_TYPE_NONE and isset($_POST["conduct_{$row['Username']}"])) {
			...
			if($comment_type == $COMMENT_TYPE_MANDATORY or $comment_type == $COMMENT_TYPE_OPTIONAL) {
				...
				if(isset($_POST["cval_{$row['Username']}"])) {
					...
					if(strval(intval($cval)) != $cval) $cval = "";
				} else {
					$comment = NULL;
				}
			}
			...
			if($_POST['action'] == "Apply conduct to all my subjects" and in_array($username, $teachers)) {
