...
if ($_POST['action'] == "Yes, I'm finished" or
	...
	if ($_POST['action'] == "Yes, I'm finished") {
		$query = "SELECT classlist.ClassTermIndex, user.Gender, user.FirstName, user.Surname, " .
			 ...
			 "       classlist.ReportDone " . "       FROM user, classlist " .
...
if ($is_principal and $pr_comment_type != $COMMENT_TYPE_NONE and
	 ! $row['PrincipalCommentDone']) {
if ($row['PrincipalComment'] != "NULL" or
...
if (is_null($row['PrincipalComment']) and
	 $pr_comment_type == $COMMENT_TYPE_MANDATORY) {
