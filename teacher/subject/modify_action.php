...
if ($is_teacher or $is_admin) {
	...
	if ($_POST["action"] == "Update" or
		 $_POST["action"] == "Apply to all my subjects") {
		...
		if (($teacher_can_modify and $can_modify) or $is_admin) {
			...
			while ( $aRow = & $aRes->fetchRow(DB_FETCHMODE_ASSOC) ) {
				...
				if (isset($_POST["weight_$cat_list_index"])) {
					...
					$query = "UPDATE categorylist " . "SET Weight=$weight " .
		...
		if ($_POST["action"] == "Apply to all my subjects") {
			...
			log_event($LOG_LEVEL_TEACHER, "teacher/subject/modify_action.php", 
					$LOG_TEACHER, "Modified options for subject $subject.");
		...
		if (($teacher_can_modify == "1" and $can_modify) or $is_admin) {
			if ($_POST["action"] == "Add") { // Check for added categories
				...
				$error = add_category_to_subject($subjectindex, 
												$_POST['name_add'], 
												floatval($_POST['weight_add']));
			 ...
			 "WHERE SubjectIndex=$subjectindex";
				...
				while ( $row = & $aRes->fetchRow(DB_FETCHMODE_ASSOC) ) {
					if (isset($_POST[$row['CategoryListIndex']]) and
						 $_POST[$row['CategoryListIndex']] == "Remove") {
						$messages = remove_category_from_subject($subjectindex, 
																$row['CategoryListIndex']);
						$errorlist = $messages;
						break;
					}
				}
	...
	log_event($LOG_LEVEL_ERROR, "teacher/subject/modify_action.php", 
			$LOG_DENIED_ACCESS, 
