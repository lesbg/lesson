...
if ($is_teacher or $is_admin) {
    ...
    if ($_POST["action"] == "Yes, apply options") {
        ...
        while ( $row = & $res->fetchRow(DB_FETCHMODE_ASSOC) ) {
            ...
            if (($new_teacher_can_modify and $new_can_modify) or $is_admin) {
                ...
                while ( $aRow = & $aRes->fetchRow(DB_FETCHMODE_ASSOC) ) {
                    remove_category_from_subject($new_subjectindex,
                                                $aRow['CategoryListIndex']);
                }
                ...
                if (DB::isError($aRes))
                    die($aRes->getDebugInfo());
                while ( $aRow = & $aRes->fetchRow(DB_FETCHMODE_ASSOC) ) {
                    add_category_to_subject($new_subjectindex,
                                            $aRow['CategoryIndex'], 0);
                }
        ...
        log_event($LOG_LEVEL_TEACHER, "teacher/subject/apply_to_all.php",
                $LOG_TEACHER,
    ...
    log_event($LOG_LEVEL_ERROR, "teacher/subject/apply_to_all.php",
            $LOG_DENIED_ACCESS,
