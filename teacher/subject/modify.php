<?php
	...
	$link            = "index.php?location=" . dbfuncString2Int("teacher/subject/modify_action.php") .
					   "&amp;key=" .           $_GET['key'] .
	...
	if($is_teacher or $is_admin) {
		...
		$res =&  $db->query("SELECT ShowAverage, RenameUploads, CanModify, SubjectTypeIndex, " .
		...
		if ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
			...
			if($is_admin or $teacher_can_modify) {
				...
				if($res->numRows() > 0) {
					while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
						...
						if($row['TotalWeight'] > 0) {
							$percentage = sprintf("%01.1f", $row['Weight'] / $row['TotalWeight'] * 100) . "%";
						} else {
							$percentage = "N/A";
						}
						echo "            <tr>\n";
				...
				if($is_admin or $can_modify) {
					...
					$this_option = str_replace("<option value='{$_POST['name_add']}'>", "<option value='{$_POST['name_add']}'}' selected>", $options);
					echo "               <td><select name='name_add'>\n";
					echo "                      <option value=''></option>\n";
					echo "                      $this_option";
					echo "                   </select>\n";
					echo "               </td>\n";
					if(!isset($_POST['weight_add'])) $_POST['weight_add'] = "";
		...
		log_event($LOG_LEVEL_ERROR, "teacher/subject/modify.php", $LOG_DENIED_ACCESS,
