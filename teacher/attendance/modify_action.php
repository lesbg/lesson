<?php
	...
	if($res->numRows() > 0 or dbfuncGetPermission($permissions, $PERM_ADMIN)) {
		if($_POST["action"] == "Done")  {
			...
			$pres =&  $db->query("SELECT Username FROM disciplineperms " .
								"WHERE Permissions >= $PUN_PERM_SUSPEND");
			...
			while ($row =& $res->fetchRow(DB_FETCHMODE_ASSOC)) {
				if(isset($_POST["att_{$row['Username']}"])) {
					...
					$valid_input = false;
					if($value == $ATT_ABSENT or $value == $ATT_LATE or $value == $ATT_IN_CLASS) $valid_input = true;
					if($value == $ATT_SUSPENDED and $is_admin) $valid_input = true;
					if($valid_input) {
		...
		log_event($LOG_LEVEL_ERROR, "teacher/attendance/modify_action.php", $LOG_DENIED_ACCESS,
